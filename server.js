...
function DetentionTracker({ load, me }) {
  // Continuous browser GPS watching was removed — it was unreliably
  // reporting wildly wrong distances (hundreds of miles off) for some
  // carriers, which made the whole geofence check untrustworthy. Manual
  // coordinate entry, verified against the exact same backend geofence math,
  // is now the only (and more reliable) way to check in.
  ...
  // "I've Arrived" — carrier enters their real coordinates, verified against
  // the exact same backend geofence math as before, just as a single
  // checked snapshot rather than continuous (and unreliable) GPS watching.
  const checkIn = async () => {
    ...
    setCheckingIn(true);
    try {
      // Real geocoding — the previous mock geocoder (zipToCoord) produced a
      // pseudo-random coordinate with no actual relationship to the real
      // ZIP code, which is exactly what was causing the "621 miles away"
      // readings regardless of how close someone actually was.
      ...
      if (data?.arrivalAt) {
        ...
        setManualError(`You appear to be ${data?.distMiles ?? "several"} miles from the facility — check-in requires being within about 5 miles of the site. Double-check your coordinates and try again once you've arrived.`);
      }
    } catch (e) {
      setManualError("Could not reach the server. Check your connection and try again.");
    } finally {
      setCheckingIn(false);
    }
  };
        ...
        <div style={{ border: load.requirements?.requireGpsTracking ? "1px solid #FFD98C" : "1px solid #E2DCCC", background: load.requirements?.requireGpsTracking ? "#FFF6E5" : "#fff", borderRadius: 8, padding: "10px 14px" }}>
          <div style={{ fontSize: 12, color: "#6B6557", display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
            ...
            {load.requirements?.requireGpsTracking
              ...
              : "Enter your coordinates once you've arrived on-site to enable automatic detention pay for this load."}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
            <Field label="Latitude"><input value={manualLat} onChange={(e) => setManualLat(e.target.value)} placeholder="e.g. 39.9612" style={{ ...inputStyle, width: 130 }} /></Field>
            <Field label="Longitude"><input value={manualLng} onChange={(e) => setManualLng(e.target.value)} placeholder="e.g. -82.9988" style={{ ...inputStyle, width: 130 }} /></Field>
            <button onClick={checkIn} disabled={checkingIn} style={{ ...primaryBtn("#FFB400"), padding: "9px 16px", display: "flex", alignItems: "center", gap: 6, opacity: checkingIn ? 0.7 : 1 }}>
              <Navigation size={13} /> {checkingIn ? "Checking…" : "I've Arrived"}
            </button>
          </div>
          <div style={{ fontSize: 10, color: "#9A958A", marginTop: 6 }}>
            Tip: search "my coordinates" on your phone's maps app to find these.
          </div>
          {manualError && <div style={{ fontSize: 11, color: "#C0432B", marginTop: 8 }}>{manualError}</div>}
        </div>
      )}
...
function TruckerApp({ me, corp, loads, messages, independentShippers, corporations, setLoads, updateTrucker, updateShipperProfile, addCorpMember, sendMessage, setMessages, addNotification, onLogout, onSwitchProfile }) {
  ...
  const acceptEscrowWaiver = async () => {
    ...
    try {
      ...
      console.warn("Payment hold acknowledgment not saved to DB:", err.message);
    }
  };
  const cancelLoad = async (loadId, reason) => {
    ...
    <Shell title={me.name} subtitle={corp ? corp.companyName : me.companyLabel} avatar={me.avatar} me={me} onLogout={onLogout} onSwitchProfile={onSwitchProfile}
      ...
      {tab === "mine" && (
        <MyHauls loads={myLoads} me={me} resolveShipper={resolveShipper} onOpenChat={(loadId, shipperId) => { markRead(threadKey(loadId, me.id)); setChatWith({ loadId, shipperId }); }} unreadCounts={unreadCounts}
          onUploadDoc={addDocument} onRateShipper={rateShipper} onConfirmDeliveryStatus={confirmDeliveryStatus} onRequestQuickPay={requestQuickPay} onCancelLoad={cancelLoad} onMarkDelivered={markDelivered} onMarkPickedUp={markPickedUp} />
      )}
      {tab === "messages" && (
        <MessagesInbox
          me={me}
...
function MyHauls({ loads, me, resolveShipper, onOpenChat, onUploadDoc, onRateShipper, onConfirmDeliveryStatus, onRequestQuickPay, onCancelLoad, onMarkDelivered, onMarkPickedUp }) {
