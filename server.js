          ...
          {[
            ...
            },
            {
              ...
              tiers: [
                ...
                { name: "Enterprise", price: "$3,500/mo", discounted: "$2,800/mo", profiles: "151–500 profiles" },
              ],
            },
          ].map(f => (
            <div key={f.title} style={{ background: "#2A2D32", border: "1px solid #44484D", borderRadius: 12, padding: "22px 20px", flex: 1, minWidth: 210, textAlign: "left" }}>
              ...
              {f.tiers && (
                <div style={{ fontSize: 10, color: "#9A958A", marginTop: 8 }}>
                  500+ profiles? <button onClick={() => setPage("companies")} style={{ background: "none", border: "none", color: "#FFB400", fontWeight: 700, fontSize: 10, cursor: "pointer", padding: 0 }}>Contact us</button> for custom pricing.
                </div>
              )}
            </div>
          ))}
...
// ─────────────────────────────────────────────────────────────────────
// The operator PIN used to be hardcoded right here as plain text — visible
// to anyone who opened browser dev tools and searched the compiled bundle.
// It's now verified by the backend only (see tryOperatorLogin below), and
// the real value lives solely in Railway's OPERATOR_PIN environment variable.
...
// ── Stripe (test mode) ──
const STRIPE_PUBLISHABLE_KEY = "pk_test_51TvkOqLjir4QqWRzdQ2fmQxEQybW158zAM00BmJ7jf5sidfZ0N5g4dUe5hLpkbkJdOncp9Ve41MngshyUIiQNl8400ExfUkEjj";

function startStripeConnect(userId, corpId, email) {
  const params = new URLSearchParams({ userId, returnOrigin: window.location.origin });
...
async function startStripeCheckout({ userId, email, planId, billingCycle, planLabel }) {
  try {
    const res = await api.post("/api/stripe/create-checkout-session", { userId, email, planId, billingCycle, planLabel, returnOrigin: window.location.origin });
...
const api = {
  async post(path, body) {
    const r = await fetch(`${API_URL}${path}`, { method: "POST", headers: { "Content-Type": "application/json", ...(operatorPin ? { "x-operator-pin": operatorPin } : {}), ...(sessionToken ? { "x-session-token": sessionToken } : {}) }, body: JSON.stringify(body) });
    if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.error || `API error ${r.status}`); }
    return r.json();
  },
  async get(path) {
    const r = await fetch(`${API_URL}${path}`, { headers: { ...(operatorPin ? { "x-operator-pin": operatorPin } : {}), ...(sessionToken ? { "x-session-token": sessionToken } : {}) } });
    if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.error || `API error ${r.status}`); }
    return r.json();
  },
  async patch(path, body) {
    const r = await fetch(`${API_URL}${path}`, { method: "PATCH", headers: { "Content-Type": "application/json", ...(operatorPin ? { "x-operator-pin": operatorPin } : {}), ...(sessionToken ? { "x-session-token": sessionToken } : {}) }, body: JSON.stringify(body) });
    if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.error || `API error ${r.status}`); }
    return r.json();
  },
  async del(path) {
    const r = await fetch(`${API_URL}${path}`, { method: "DELETE", headers: { ...(operatorPin ? { "x-operator-pin": operatorPin } : {}), ...(sessionToken ? { "x-session-token": sessionToken } : {}) } });
    if (!r.ok) { const e = await r.json().catch(() => ({})); throw new Error(e.error || `API error ${r.status}`); }
    return r.json();
  },
};
...
const COMPANY_TIERS = [
  ...
  { id: "enterprise", name: "Enterprise", range: "151–500 profiles",   price: 3500 },
];
...
export default function HaulBoard() {
  ...
  useEffect(() => {
    ...
    if (connectResult) {
      ...
      else if (connectResult === "incomplete") alert("You started connecting Stripe but didn't finish the onboarding form. Head back to your Profile page anytime to pick up where you left off.");
  ...
  const setSession = (next) => {
    setSessionRaw(next);
    try {
      if (next && next.role !== "operator") {
        localStorage.setItem("df_session", JSON.stringify(next));
        if (sessionToken) localStorage.setItem("df_session_token", sessionToken);
      } else {
  ...
  const [verifyingOpPin, setVerifyingOpPin] = useState(false);
  const tryOperatorLogin = async () => {
    setVerifyingOpPin(true);
    try {
      const { valid } = await api.post("/api/operator/verify-pin", { pin: opPin });
      if (valid) {
        operatorPin = opPin;
        setShowOperatorLogin(false); setOpPin(""); setSession({ role: "operator" });
        try {
          const { users } = await api.get("/api/operator/users");
          const dbShippers = users.filter(u => u.role === "shipper").map(dbUserToFrontend);
          const dbTruckers = users.filter(u => u.role === "trucker").map(dbUserToFrontend);
          const dbCorps    = users.filter(u => u.role === "corp").map(dbUserToCorp);
          setIndependentShippers(dbShippers);
          setIndependentTruckers(dbTruckers);
          setCorporations(dbCorps);
        } catch (err) {
          console.warn("Could not load full operator data:", err.message);
        }
      } else {
        alert("Incorrect PIN."); setOpPin("");
      }
    } catch (err) {
      alert(err.message || "Could not verify PIN — check your connection.");
    } finally {
      setVerifyingOpPin(false);
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (valid) {
          setSessionRaw(parsed);
          const savedToken = localStorage.getItem("df_session_token");
          if (savedToken) sessionToken = savedToken;
        }
  ...
  // Real progress and delivery are driven by the carrier's own actions —
  // GPS tracking while en route, and a manual "Mark as Delivered" button on
  // arrival. There is no automatic timer-based progress simulation.
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: 28, width: 320, boxShadow: "0 12px 40px rgba(0,0,0,0.4)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={tryOperatorLogin} disabled={verifyingOpPin} style={{ ...primaryBtn("#1B1D21"), flex: 1, opacity: verifyingOpPin ? 0.7 : 1 }}>{verifyingOpPin ? "Verifying…" : "Enter"}</button>
    ...
    return (
      <>
        <AuthGate
          ...
          onIndieShipperLogin={(user) => {
            const mappedUser = dbUserToFrontend(user);
            setIndependentShippers((p) => upsert(p, mappedUser));
            setSession({ role: "shipper", corpId: null, profileId: mappedUser.id });
          }}
          onIndieTruckerLogin={(user) => {
            const mappedUser = dbUserToFrontend(user);
            setIndependentTruckers((p) => upsert(p, mappedUser));
            setSession({ role: "trucker", corpId: null, profileId: mappedUser.id });
          }}
          onIndieShipperAuth={async (s) => {
            try {
              const { user, sessionToken: newToken } = await api.post("/api/auth/signup", { ...s, role: "shipper" });
              ...
              if (err.message?.includes("already exists")) {
                // Never auto-login into an existing account just because
                // signup hit a duplicate email — direct to the real secure
                // login instead of silently signing in as whoever owns it.
                alert("An account with this email already exists. Please use the \"Log in\" option instead, which will email you a secure one-time code.");
              } else {
                // 🧪 TEST MODE — backend unreachable, create account locally so testing isn't blocked
                console.warn("Backend unreachable — creating local test account:", err.message);
          ...
          onIndieTruckerAuth={async (t) => {
            try {
              const { user, sessionToken: newToken } = await api.post("/api/auth/signup", { ...t, role: "trucker" });
              sessionToken = newToken;
              const mappedUser = dbUserToFrontend(user);
              setIndependentTruckers((p) => upsert(p, mappedUser));
              startEmailVerification(mappedUser.email, mappedUser.name, { role: "trucker", corpId: null, profileId: mappedUser.id });
              startStripeConnect(mappedUser.id, null, mappedUser.email);
            } catch (err) {
              if (err.message?.includes("already exists")) {
                alert("An account with this email already exists. Please use the \"Log in\" option instead, which will email you a secure one-time code.");
              } else {
    ...
    return (
      <OperatorApp
        ...
        onRemoveShipper={async (id) => {
          try {
            await api.del(`/api/operator/users/${id}`);
            setIndependentShippers((p) => p.filter((s) => s.id !== id));
          } catch (e) { alert("Could not delete this shipper: " + e.message); }
        }}
        ...
        onRemoveTrucker={async (id) => {
          ...
          } catch (e) { alert("Could not delete this carrier: " + e.message); }
        }}
        ...
        onRemoveCorp={async (id) => {
          ...
          } catch (e) { alert("Could not delete this company: " + e.message); }
        }}
        ...
        onRemoveLoad={async (id) => {
          ...
          try { await api.del(`/api/loads/${id}`); }
          catch (err) { console.warn("Load delete not saved to DB:", err.message); }
        }}
        onImpersonate={(newSession) => setSession({ ...newSession, impersonating: true })}
        onLogout={() => { operatorPin = null; setSession(null); }}
      />
    );
  }
  ...
  const recheckInsurance = async (mcNumber) => {
    ...
    if (corp) {
      setCorporations((prev) => prev.map((c) => (c.id === corp.id ? { ...c, verification: data } : c)));
      try { await api.patch(`/api/auth/user/${corp.id}`, { verification: data }); }
      catch (err) { console.warn("Re-verification not saved to DB:", err.message); }
    } else {
      updateTrucker(me.id, (t) => ({ ...t, verification: data }));
      try { await api.patch(`/api/auth/user/${me.id}`, { verification: data }); }
      catch (err) { console.warn("Re-verification not saved to DB:", err.message); }
    }
  };
...
// ====================================================================
function AuthGate({ independentShippers, independentTruckers, corporations, onIndieShipperAuth, onIndieTruckerAuth, onIndieShipperLogin, onIndieTruckerLogin, onCorpAuth }) {
      ...
      {page === "shippers" && (
        ...
        >
          <LoginForm role="shipper" accent="#FFB400" existing={independentShippers} embedded
            ...
            onLoginSuccess={onIndieShipperLogin} />
        </PersonaPage>
      )}
      {page === "carriers" && (
        <PersonaPage
        ...
        >
          <LoginForm role="trucker" accent="#FF5A1F" existing={independentTruckers} embedded
            ...
            onLoginSuccess={onIndieTruckerLogin} />
        </PersonaPage>
      )}
      {page === "companies" && (
        <PersonaPage
          ...
          bullets={[
            ...
            "Fleets up to 500 drivers stay on a flat Enterprise rate — no custom quotes or sales calls required. Bigger than that? We'll work out custom pricing with you directly",
          ]}
...
function TierTable({ accent, highlightId, onSelectTier }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 360 }}>
      ...
      <div style={{ fontSize: 11, color: "#9A958A", marginTop: 2 }}>One flat bill per company — no per-seat charges. Upgrade automatically as your roster grows past a threshold.</div>
      <div style={{ fontSize: 12, color: "#44484D", marginTop: 6, background: "#F8F5EE", border: "1px solid #E2DCCC", borderRadius: 6, padding: "8px 10px" }}>
        Running more than 500 profiles? <a href="mailto:ashton@directfreightco.com?subject=Enterprise pricing — 500+ profiles" style={{ color: "#B5790A", fontWeight: 700 }}>Contact us</a> for custom pricing.
      </div>
    </div>
  );
}
...
function LoginForm({ role, accent, existing, onBack, onSubmit, onLoginSuccess, embedded }) {
  ...
  const [verifyingLoginCode, setVerifyingLoginCode] = useState(false);
  ...
  const requestLoginCode = async () => {
    ...
    try {
      ...
      setLoginError(err.message || "Could not send a login code. Try again.");
    } finally {
  ...
  const verifyLoginCode = async () => {
    if (!loginCode.trim()) { setLoginError("Enter the code we emailed you."); return; }
    setLoginError("");
    setVerifyingLoginCode(true);
    try {
      const { user, sessionToken: newToken } = await api.post("/api/auth/verify-login-code", { email, code: loginCode });
      sessionToken = newToken;
      // Real, already-existing, already-verified account — this must never
      // go through the same path as a fresh signup attempt. Previously it
      // did, which meant a genuinely successful login would immediately
      // trigger an unwanted extra signup call right afterward, hit the
      // expected 409 for an account that (correctly) already exists, and
      // show a confusing "already exists" message even though login itself
      // had just fully succeeded.
      (onLoginSuccess || onSubmit)(user);
    } catch (err) {
      if (err.message?.includes("429") || err.message?.toLowerCase().includes("too many")) {
        setLoginError("Too many attempts — please wait about 15 minutes before trying again.");
      } else {
        setLoginError(err.message || "Incorrect or expired code.");
      }
    } finally {
      setVerifyingLoginCode(false);
    }
  };
  ...
  const trySignup = () => {
    ...
    // Real Stripe Connect/Checkout requires a real account to exist first —
    // so the account is created here, and the very next step (handled by
    // the parent's onSubmit) sends them straight to Stripe to actually
    // connect a payout account or start a subscription.
    onSubmit({
    ...
    <Card style={{ width: 440, maxWidth: "100%", padding: 32 }}>
      ...
      {mode === "login" ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {!loginCodeSent ? (
            <>
              <Field label="Email"><input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" style={inputStyle} onKeyDown={(e) => e.key === "Enter" && requestLoginCode()} /></Field>
              <div style={{ fontSize: 11, color: "#9A958A" }}>We'll email you a one-time code — no password needed.</div>
              <button onClick={requestLoginCode} disabled={sendingLoginCode} style={{ ...primaryBtn(accent), marginTop: 8, opacity: sendingLoginCode ? 0.7 : 1 }}>
                {sendingLoginCode ? "Sending code…" : "Send login code"}
              </button>
            </>
          ) : (
            <>
              <div style={{ fontSize: 12, color: "#3E7A4B", background: "#F1F8F2", border: "1px solid #BFE0C6", borderRadius: 6, padding: "8px 10px" }}>
                Code sent to <b>{email}</b> — check your inbox.
              </div>
              <Field label="6-digit code"><input value={loginCode} onChange={(e) => setLoginCode(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="123456" style={inputStyle} onKeyDown={(e) => e.key === "Enter" && verifyLoginCode()} /></Field>
              <button onClick={verifyLoginCode} disabled={verifyingLoginCode} style={{ ...primaryBtn(accent), marginTop: 8, opacity: verifyingLoginCode ? 0.7 : 1 }}>
                {verifyingLoginCode ? "Verifying…" : "Verify & log in"}
              </button>
              <button onClick={() => { setLoginCodeSent(false); setLoginCode(""); setLoginError(""); }} style={{ ...ghostBtn, fontSize: 12 }}>
                Use a different email
              </button>
            </>
          )}
          {loginError && <div style={{ fontSize: 12, color: "#C0432B" }}>{loginError}</div>}
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {/* Real Stripe connection happens immediately after account
              creation below — Connect and Checkout both require a real
              account to exist first, so there's nothing to fill in here. */}
          <div style={{ borderTop: "1px solid #EEE8DA", paddingTop: 12 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#44484D", marginBottom: 6, display: "flex", alignItems: "center", gap: 5 }}>
              <CreditCard size={13} color="#635BFF" />
              {role === "trucker" ? "Next: connect your Stripe payout account" : "Next: add a payment method"}
            </div>
            <div style={{ fontSize: 11, color: "#6B6557", lineHeight: 1.5 }}>
              {role === "trucker"
                ? "Right after you create your account, you'll be sent to Stripe's own secure page to connect your bank account. Direct Freight Co never sees your bank details."
                : "Right after you create your account, you'll be sent to Stripe's own secure checkout to add a payment method. Your card is charged for load payments, detention fees, and your subscription — never stored on our servers."}
            </div>
          </div>

          <TermsCheckbox checked={agreedToTerms} onChange={setAgreedToTerms} onOpenTerms={() => setShowTerms(true)} role={role} />
          <button onClick={trySignup} disabled={!agreedToTerms || !bizVerified} style={{ ...primaryBtn(accent), marginTop: 8, opacity: agreedToTerms && bizVerified ? 1 : 0.5 }}>
            Create account
          </button>
...
const EMAILJS_PUBLIC_KEY  = "u60FgyRpbv045jQOB";    // e.g. "abc123xyz"
...
function QuickPayPanel({ load, me, onRequestQuickPay }) {
  // QuickPay lets a carrier convert an already-released standard payout into
  // an instant one (for a fee) — it should never be available before the
  // shipper has actually released payment. Requiring load.paid here (not
  // the reverse) is the fix for that.
  if (load.status !== "delivered" || !load.paid || load.quickPay || !me.payout?.connected) return null;
...
// ====================================================================
// PAYMENT TERMS AUTHORIZATION (shown once on first load acceptance)
// ====================================================================
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(27,29,33,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 80, padding: 16 }}>
      <Card style={{ width: "100%", maxWidth: 520, padding: 28 }}>
        <div style={{ fontFamily: "Oswald, sans-serif", fontWeight: 700, fontSize: 18, textTransform: "uppercase", marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
          <ShieldCheck size={18} color="#FFB400" /> Payment Terms Authorization
        </div>
        <div style={{ fontSize: 13, color: "#44484D", lineHeight: 1.6, marginBottom: 16 }}>
          Direct Freight never takes custody, possession, or control of any load payment funds — all payments are processed and held solely by Stripe, a licensed third-party payment processor. By proceeding with your first load on this platform, you acknowledge and agree that:
          <ul style={{ marginTop: 8, paddingLeft: 18 }}>
            <li>Disputed load payments may have their release from Stripe paused, at Direct Freight's instruction, pending resolution between parties.</li>
            <li>You waive immediate chargeback rights on amounts subject to such a pause while a documented dispute is active.</li>
        ...
        <label style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 12, color: "#44484D", cursor: "pointer", marginBottom: 14 }}>
          ...
          I understand and agree to the payment terms described above.
        </label>
...
function SubscriptionPortalPanel({ isCorp, corp, me, onSave }) {
  ...
  const openBillingPortal = async () => {
    const customerId = corp?.billing?.stripeCustomerId || me?.billing?.stripeCustomerId;
    if (!customerId) {
      alert("You don't have a payment method on file yet through real Stripe billing — this becomes available once you've completed checkout with a card.");
      return;
    }
    setOpeningPortal(true);
    try {
      const { url } = await api.post("/api/stripe/create-billing-portal-session", {
        customerId, returnOrigin: window.location.origin,
      });
    ...
    <div id="subscription-panel" style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 520 }}>
      ...
      <Card style={{ padding: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <button onClick={openBillingPortal} disabled={openingPortal} style={{ ...ghostBtn, justifyContent: "flex-start", display: "flex", gap: 6, alignItems: "center", opacity: openingPortal ? 0.7 : 1, width: "100%", textAlign: "left" }}>
            <CreditCard size={14} /> {openingPortal ? "Opening…" : "Update payment method"}
          </button>
          <button onClick={openBillingPortal} disabled={openingPortal} style={{ ...ghostBtn, justifyContent: "flex-start", display: "flex", gap: 6, alignItems: "center", opacity: openingPortal ? 0.7 : 1, width: "100%", textAlign: "left" }}>
            <FileText size={14} /> {openingPortal ? "Opening…" : "View & download invoices"}
          </button>
          <button onClick={openBillingPortal} disabled={openingPortal} style={{ ...ghostBtn, color: "#FF5A1F", justifyContent: "flex-start", display: "flex", gap: 6, alignItems: "center", opacity: openingPortal ? 0.7 : 1, width: "100%", textAlign: "left" }}>
            <X size={14} /> {openingPortal ? "Opening…" : "Cancel subscription"}
          </button>
        </div>
      </Card>
...
function MyAccountPanel({ me, corp, isCorp, onSave, onGoBilling, onDeleteAccount }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 560 }}>
      ...
      <Card style={{ padding: 24, border: "1px solid #F3B7A6" }}>
        <div style={{ fontFamily: "Oswald, sans-serif", fontWeight: 700, fontSize: 16, textTransform: "uppercase", marginBottom: 6, color: "#C0432B" }}>Danger Zone</div>
        {!showDeleteConfirm ? (
          <>
            <div style={{ fontSize: 13, color: "#6B6557", marginBottom: 14, lineHeight: 1.5 }}>
              Permanently delete your account and profile. This can't be undone — your load history, ratings, and any saved information will be gone.
            </div>
            <button onClick={() => setShowDeleteConfirm(true)} style={{ background: "transparent", border: "1px solid #C0432B", color: "#C0432B", borderRadius: 8, padding: "10px 18px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
              Delete My Account
            </button>
          </>
        ) : (
          <div style={{ background: "#FDF1EE", border: "1px solid #F3B7A6", borderRadius: 8, padding: 16 }}>
            <div style={{ fontSize: 13, color: "#8A3322", fontWeight: 700, marginBottom: 10 }}>
              This is permanent. Type DELETE below to confirm you want to permanently remove your account.
            </div>
            <input
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={async () => {
                  ...
                  catch (err) { alert("Could not delete account: " + err.message); setDeleting(false); }
                }}
                disabled={deleteConfirmText !== "DELETE" || deleting}
                style={{ background: "#C0432B", color: "#fff", border: "none", borderRadius: 8, padding: "10px 18px", fontWeight: 700, fontSize: 13, cursor: deleteConfirmText === "DELETE" ? "pointer" : "not-allowed", opacity: deleteConfirmText === "DELETE" ? 1 : 0.5 }}
              >
                {deleting ? "Deleting…" : "Permanently Delete"}
              </button>
              <button onClick={() => { setShowDeleteConfirm(false); setDeleteConfirmText(""); }} disabled={deleting} style={ghostBtn}>
                Cancel
              </button>
            </div>
          </div>
...
function CorpLoginForm({ subtype, existing, onBack, onSubmit, embedded, selectedTier }) {
  ...
  const requestLoginCode = async () => {
    if (!email.trim()) { setLoginError("Enter your company email first."); return; }
    setLoginError("");
    setSendingLoginCode(true);
    try {
      await api.post("/api/auth/request-login-code", { email });
  ...
  const verifyLoginCode = async () => {
    ...
    try {
      const { user, sessionToken: newToken } = await api.post("/api/auth/verify-login-code", { email, code: loginCode });
      sessionToken = newToken;
      if (user.role !== "corp") { setLoginError("No company account found with that email."); return; }
  ...
  const trySignup = async () => {
    ...
    try {
      const { user, sessionToken: newToken } = await api.post("/api/auth/signup", corpData);
      sessionToken = newToken;
      ...
      if (err.message?.includes("already exists")) {
        // Never auto-login into an existing account just because signup
        // hit a duplicate email — direct to the real secure login instead.
        alert("A company account with this email already exists. Please use the \"Log in\" option instead, which will email you a secure one-time code.");
        return;
      }
      // 🧪 TEST MODE — backend unreachable, create account locally so testing isn't blocked
      console.warn("Backend unreachable — creating local test company account:", err.message);
      onSubmit({
        id: "local_" + Date.now(),
    ...
    <Card style={{ width: 480, maxWidth: "100%", padding: 32 }}>
      ...
      {mode === "login" ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {!loginCodeSent ? (
            <>
              <Field label="Company email"><input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="dispatch@yourcompany.com" style={inputStyle} onKeyDown={(e) => e.key === "Enter" && requestLoginCode()} /></Field>
              <div style={{ fontSize: 11, color: "#9A958A" }}>We'll email you a one-time code — no password needed.</div>
              <button onClick={requestLoginCode} disabled={sendingLoginCode} style={{ ...primaryBtn(accent), marginTop: 8, opacity: sendingLoginCode ? 0.7 : 1 }}>
                {sendingLoginCode ? "Sending code…" : "Send login code"}
              </button>
    ...
    <div>
      <label style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 12, color: "#44484D", cursor: "pointer" }}>
        ...
        <span>
          ...
          {isCarrier && <>, and <LinkBtn onClick={() => setShowCarrier(true)}>Carrier Agreement</LinkBtn> (including mandatory location tracking, double-brokering prohibition, and payment terms)</>}
...
const TOS_FULL_TEXT = `# Direct Freight Terms of Service

**Last updated: July 27, 2026**

These Terms of Service ("Terms") govern access to and use of the Direct Freight website, applications, and related services (collectively, the "Platform"), operated by Direct Freight Co LLC ("Direct Freight," "we," "us," or "our"). By creating an account, accessing, or using the Platform, you ("you," "User") agree to be bound by these Terms. If you are agreeing on behalf of a company, you represent that you have authority to bind that company, and "you" refers to that company.

If you do not agree to these Terms, do not use the Platform.

---

## 1. What Direct Freight Is — and Is Not

1.1 Direct Freight is a **technology platform** that allows Shippers to post freight and Truckers (independent owner-operators and trucking companies) to view, bid on, and accept those loads directly, without a freight broker as an intermediary.

1.2 **Direct Freight is not a freight broker, motor carrier, freight forwarder, shipper, insurer, or financial institution.** Direct Freight does not book, arrange, broker, or forward any shipment; does not own, lease, or operate any truck or trailer; does not employ, dispatch, or supervise any driver; and does not take possession, custody, or control of any freight at any time. Direct Freight is not a party to any transportation contract, bill of lading, or rate agreement formed between a Shipper and a Trucker — it provides only the software venue through which independent Users find and contract with one another.

1.3 Because Direct Freight is solely a technology venue and not a transportation provider, **Direct Freight bears no responsibility for cargo loss, theft, damage, delay, or any accident, injury, or property damage arising during transportation of a shipment arranged through the Platform.** Any such claim lies solely between the Shipper and the Trucker (and their respective cargo, liability, or other insurance carriers), and Users agree not to name Direct Freight as a party to any claim, demand, or lawsuit arising from the physical transportation of freight.

1.4 All agreements regarding rate, route, cargo, delivery terms, claims, damages, detention, and payment are solely between the Shipper and the Trucker. Direct Freight is not responsible for, and disclaims all liability arising from, the performance or non-performance of either party under such agreements.

1.5 Nothing in this Platform constitutes a guarantee of a carrier's operating authority, safety record, or insurance coverage at any given moment. See Section 6.

---

## 2. Eligibility and Account Types

2.1 You must be at least eighteen (18) years old and have the legal authority to enter into contracts to use the Platform.

2.2 The Platform supports the following account types:
- **Independent Shipper** — an individual or business shipping freight under its own name.
- **Independent Trucker** — an owner-operator hauling freight, optionally under their own operating authority.
- **Corporate Account (Trucking or Shipping)** — a company account under which multiple individual driver or team-member profiles operate under one subscription.

2.3 If you create a Corporate Account, you are responsible for all activity conducted under every profile associated with that account, including drivers or team members you add, and for ensuring each individual profile holder is authorized to act on the company's behalf.

2.4 You agree to provide accurate, current information, including legal business name, contact information, USDOT/MC numbers (if applicable), and insurance information, and to promptly update this information if it changes.

---

## 3. Subscriptions and Billing

3.1 Access to the Platform requires an active paid subscription. Subscription plans, pricing, and included features are as displayed at checkout and may be updated by Direct Freight from time to time, with notice to affected Users before changes take effect for existing subscriptions.

3.2 **Corporate Accounts are billed a single flat subscription fee based on their fleet/team-size tier**, as displayed on the Platform at the time of signup and in your account's Billing section. This fee covers every individual driver or team-member profile within that tier's stated range — it is not a per-driver or per-seat charge. If your roster grows beyond your current tier's range, your account will automatically move to the next applicable tier, up to the top Enterprise tier (151–500 profiles), which remains a flat self-serve rate within that range. Your subscription fee will be adjusted accordingly, with notice provided before the change takes effect on your next billing cycle. Fleets or teams exceeding 500 profiles should contact Direct Freight directly to arrange custom pricing. Direct Freight reserves the right to update tier pricing or thresholds from time to time, with advance notice to affected Users before changes take effect for existing subscriptions.

3.3 Subscriptions renew automatically on a recurring basis until canceled. You may cancel at any time through your account settings; cancellation takes effect at the end of the current billing period, and no partial refunds are issued for early cancellation except where required by law.

3.4 Direct Freight does not charge a percentage fee, commission, or cut on individual loads, bids, or freight payments. Subscription fees are the sole charge imposed by Direct Freight for Platform access.

3.5 Payment processing for subscriptions is handled by a third-party payment processor. By subscribing, you agree to that processor's applicable terms.

---

## 4. Posting, Bidding, and Booking Loads

4.1 Shippers are solely responsible for the accuracy of load information they post, including weight, dimensions, quantity, commodity description, special handling requirements, origin/destination, mileage, and price.

4.2 Truckers are solely responsible for verifying that their equipment, licensing, and capacity are suitable for any load they bid on or accept, and for confirming all load details directly with the Shipper before transport.

4.3 Bids and counter-offers exchanged through the Platform are offers between Users. **A binding agreement is formed only when a bid, counter-offer, or listed price is affirmatively accepted** by the relevant party through the Platform. Direct Freight is not a party to that agreement and makes no representation as to its enforceability.

4.4 Direct Freight does not vet, inspect, or approve the legality or appropriateness of any cargo, route, or rate. Users are solely responsible for complying with all applicable federal, state, and local laws, including those concerning hazardous materials, weight limits, hours of service, and permitting.

---

## 5. Tracking and ETA Information

5.1 Any load tracking, progress percentage, or estimated time of arrival ("ETA") displayed on the Platform is provided for general informational purposes only and may be estimated, simulated, or derived from limited data. **Direct Freight does not guarantee the accuracy of any tracking or ETA information** and is not liable for decisions made in reliance on it.

5.2 Truckers who accept loads through the Platform consent to continuous real-time GPS location tracking while an active load is in progress, as described in the Privacy Policy and Carrier Agreement.

5.3 Location data transmitted through the Platform is used for load tracking, geofence-based detention monitoring, shipper proximity dispatch, and regulatory compliance purposes.

5.4 **Automated Detention Billing.** Direct Freight operates an automated logistics platform utilizing device-based GPS telemetry. By booking a load through the Platform, Shippers explicitly agree to the following:

&nbsp;&nbsp;&nbsp;&nbsp;(a) **Free Loading/Unloading Window.** Each booked shipment includes a standard two (2) hour free loading and unloading grace window per booked shipment, measured from the platform-validated GPS arrival timestamp at the shipper or receiver facility.

&nbsp;&nbsp;&nbsp;&nbsp;(b) **Source of Truth.** Shippers explicitly agree that Direct Freight's automated geofence logs serve as the final, contractually binding, and ultimate source of truth regarding vehicle arrival and departure times at any facility location in connection with a Platform-booked shipment.

&nbsp;&nbsp;&nbsp;&nbsp;(c) **Automatic Charge Authorization.** Shippers hereby authorize Direct Freight to automatically charge the credit card, debit card, or ACH bank account on file for any detention fees incurred past the two (2) hour grace window, billed at a flat platform rate of **$60.00 per hour**, calculated in fifteen (15) minute increments, immediately upon load departure from the facility geofence perimeter.

&nbsp;&nbsp;&nbsp;&nbsp;(d) **Finality of Charges.** All automated geofence-triggered detention charges are final and non-refundable. Disputes regarding detention charges must be submitted in writing to Direct Freight within seven (7) days of the charge date and will be reviewed against the immutable geofence log record.

&nbsp;&nbsp;&nbsp;&nbsp;(e) **Trucker Obligations.** Truckers must maintain an active mobile browser session with location permissions enabled while within the shipper or receiver facility perimeter to qualify for automated platform detention payouts. Direct Freight is not liable for detention claims where GPS tracking was not active during the dwell period.

&nbsp;&nbsp;&nbsp;&nbsp;(f) **Daily Cap.** Automated detention charges are capped at a maximum of five (5) hours (**$300.00**) per stop, regardless of actual dwell time. Any claim for detention beyond this cap must be negotiated and agreed to directly between the Shipper and Trucker outside the Platform's automated billing system; Direct Freight will not automatically charge or collect amounts beyond the cap.

&nbsp;&nbsp;&nbsp;&nbsp;(g) **Shipper Payment Authorization.** By posting a load and having a bid accepted through the Platform, the Shipper affirmatively authorizes Direct Freight (acting solely as an authorized agent for payment collection, and not as a party to the underlying freight contract) to charge the Shipper's linked payment method on file for all detention amounts calculated under this Section 5.4, up to the cap in subsection (f), without further confirmation at the time of each charge.

---

## 6. Trucker Verification, Safety Ratings, and Insurance Status

6.1 The Platform may display operating authority status, FMCSA safety ratings, and insurance status for trucking companies and independent truckers, sourced from third-party carrier verification services and/or publicly available FMCSA data ("Verification Data").

6.2 **Verification Data is provided "as-is" and may be outdated, incomplete, or inaccurate.** Verification Data reflects the information available to Direct Freight or its data providers at the time of the most recent check and may not reflect a carrier's real-time status. Direct Freight does not independently audit or guarantee the accuracy of any Verification Data.

6.3 Shippers are responsible for conducting their own independent due diligence on any Trucker before tendering freight, regardless of any Verification Data, badge, or status indicator shown on the Platform. Display of a "verified" status is not an endorsement, certification, or warranty by Direct Freight of a carrier's fitness, safety, or insurance adequacy.

6.4 **Insurance Review.** Direct Freight may, at its discretion and from time to time, review a Trucker's or trucking company's insurance status using Verification Data or self-reported documentation, and may suspend that account's access to the load board, bidding, messaging, and other Platform features if that review indicates a lapse in required coverage. Because Verification Data may be outdated, incomplete, or inaccurate (see Section 6.2), Direct Freight does not represent that insurance status is continuously or automatically monitored in real time, and any suspension under this section is a discretionary safety measure, not a determination of fault or wrongdoing. Such suspension does not relieve either party of obligations under loads already in progress, and Direct Freight is not liable for losses arising from such suspension, including loads delayed or canceled as a result.

6.5 Users are solely responsible for maintaining all legally required insurance, operating authority, and safety compliance, independent of anything displayed on the Platform.

---

## 7. Ratings and Reviews

7.1 Shippers may rate Truckers (including individual drivers under a Corporate Account) following completed deliveries. Ratings reflect the subjective opinion of the rating Shipper and are not verified or endorsed by Direct Freight.

7.2 Users may not submit false, retaliatory, or manipulated ratings, and may not offer or accept compensation in exchange for a rating.

7.3 Direct Freight may remove ratings that violate these Terms or applicable law but is under no obligation to monitor, verify, or adjudicate rating disputes.

---

## 8. Payments Between Shippers and Truckers

8.1 **Direct Freight does not process, hold, or guarantee payment for freight charges between Shippers and Truckers.** Freight payment terms, timing, and method are agreed upon directly between the parties. Where the Platform facilitates payment through a third-party payment processor (currently Stripe, Inc.), Direct Freight acts solely as a technology intermediary passing payment instructions to that processor — it does not itself hold, custody, or guarantee any funds.

8.2 **Stripe Connected Account Agreement.** To send or receive payments through the Platform, Users must create an account with Stripe and agree to the [Stripe Connected Account Agreement](https://stripe.com/legal/connect-account) (which incorporates the Stripe Services Agreement by reference), as may be amended by Stripe from time to time. By using any payment or payout feature of the Platform, you agree to be bound by Stripe's terms in addition to these Terms. Direct Freight is not a party to, and has no liability arising from, your agreement with Stripe.

8.3 **No Liability for Non-Payment or Processing Failure.** Direct Freight is not responsible for, and disclaims all liability arising from: (a) a Shipper's failure to pay, short-payment, or bank/card decline; (b) delayed, failed, or reversed transfers caused by Stripe or any linked financial institution; (c) a Trucker's failure to receive payout due to incomplete or inaccurate Stripe Connect onboarding; or (d) any other error, outage, or failure of the Stripe platform. **If a Shipper's payment method fails or a Shipper otherwise fails to pay, the Trucker's recourse is against the Shipper directly — not against Direct Freight.**

8.4 **Processing Fees and Chargebacks.** Standard Stripe payment processing fees are the responsibility of the Shipper (as the paying party) unless otherwise disclosed on the Platform at the time of the transaction. In the event of a chargeback, dispute, or reversal initiated through a Shipper's card issuer or bank, the full amount of the chargeback plus any associated Stripe chargeback fee will be the responsibility of the Shipper who initiated the payment; Direct Freight may recover such amounts by charging the Shipper's payment method on file, invoicing the Shipper directly, or offsetting future amounts owed. Truckers are not responsible for chargebacks initiated by the Shipper's financial institution absent evidence of Trucker fraud.

8.5 Any "release payment" or similar action within the Platform is a status indicator confirming the Shipper's authorization to release funds and does not itself constitute a guarantee that funds were successfully transferred; successful transfer is subject to Stripe's processing and the matters described in Section 8.3.

8.6 Users are solely responsible for resolving payment disputes, including non-payment, short-payment, or invoicing disagreements, directly with the other party and, where applicable, through Stripe's dispute process.

8.7 **Chargeback Acknowledgment and Waiver.** By accepting these Terms and by taking any action on the Platform that results in a charge to your linked payment method — including creating a subscription, posting a load, accepting a bid, or booking a load subject to automated detention billing — **you affirmatively acknowledge that you have been clearly informed of, and you consent in advance to, that charge.** This includes: recurring subscription fees and automatic renewals (Section 3); automated detention charges calculated under Section 5.4; and any other fee disclosed in these Terms or displayed on the Platform at the time of the relevant transaction. **You agree that you will not dispute, reverse, or file a chargeback for any such disclosed and authorized charge on the basis that you were unaware of, did not recognize, or did not authorize it.** Direct Freight maintains timestamped records of your acceptance of these Terms, your account activity, your load postings and bookings, and — for detention charges — the underlying GPS geofence log, and may submit these records to your card issuer or bank as evidence of your advance authorization and knowledge of the charge in response to any chargeback claim.

8.8 **Chargeback Costs and Account Consequences.** If you initiate a chargeback for a charge authorized under Section 8.7 and the chargeback is determined to be improper or unwarranted (including where Direct Freight successfully contests it with your card issuer), you agree to reimburse Direct Freight for the disputed amount plus any chargeback, dispute, or arbitration fee imposed by Stripe or your card issuer. Direct Freight may suspend your account immediately upon receiving notice of a chargeback pending its resolution, and may permanently terminate your account for repeated improper chargebacks, which Direct Freight may treat as a material breach of these Terms.

8.9 **Contact Us First.** Before initiating a chargeback with your bank or card issuer, you agree to first contact Direct Freight support in good faith to attempt to resolve the billing concern directly. This does not limit your legal right to dispute a charge with your financial institution, but your failure to first attempt resolution with Direct Freight may be considered as evidence in resolving any related dispute under Section 16.

---

## 9. Messaging

9.1 In-platform messaging is provided to facilitate communication between Shippers and Truckers regarding specific loads. Users agree not to use messaging for harassment, spam, unrelated solicitation, or any unlawful purpose.

9.2 Direct Freight does not actively monitor private messages but reserves the right to review message content in response to a complaint, legal request, or suspected violation of these Terms.
...
You agree not to:
- Provide false information about your identity, equipment, authority, insurance, or load details;
- Circumvent, manipulate, or interfere with the bidding, rating, or verification features of the Platform;
- **Contact a User found through the Platform for the purpose of moving an agreed-upon load, or a substantially similar recurring load, off-Platform to avoid subscription fees or Platform terms ("Circumvention");**
- Use the Platform to arrange transport of cargo you know to be illegal, stolen, or improperly documented;
...
- Harass, defraud, or discriminate against any other User.

---

## 11. Account Suspension and Termination

11.1 Direct Freight may suspend or terminate any account, with or without notice, for violation of these Terms — including suspected Circumvention as described in Section 10 — suspected fraud, safety risk (including lapsed insurance under Section 6.4), non-payment of subscription fees, or at our discretion to protect the integrity of the Platform.

11.2 Users may close their account at any time. Closing an account does not relieve outstanding obligations to other Users incurred before closure. **Subscription fees are non-refundable, including where an account is suspended or terminated for a violation of these Terms**, except where required by law.

---

## 12. Disclaimers

12.1 THE PLATFORM IS PROVIDED "AS IS" AND "AS AVAILABLE," WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, OR NON-INFRINGEMENT.

12.2 DIRECT FREIGHT DOES NOT WARRANT THAT THE PLATFORM WILL BE UNINTERRUPTED, ERROR-FREE, OR SECURE, OR THAT ANY VERIFICATION DATA, TRACKING INFORMATION, OR USER-SUBMITTED CONTENT IS ACCURATE OR RELIABLE.

12.3 DIRECT FREIGHT IS NOT RESPONSIBLE FOR THE ACTS, OMISSIONS, SAFETY PRACTICES, FINANCIAL CONDITION, OR LEGAL COMPLIANCE OF ANY USER, INCLUDING DAMAGE TO OR LOSS OF CARGO, VEHICLE ACCIDENTS, DELAYS, OR PERSONAL INJURY ARISING FROM TRANSPORTATION ARRANGED THROUGH THE PLATFORM.

---

## 13. Release of Claims Between Users

13.1 Because Direct Freight is a technology platform connecting Shippers and Truckers directly, and is not a party to any transportation contract, Bill of Lading, or Rate Confirmation formed between them, you release Direct Freight and its officers, employees, and affiliates from any and all claims, demands, and damages (actual and consequential) of every kind, known and unknown, arising out of or in any way connected with a dispute between you and another User — including disputes over cargo condition, payment, detention, scheduling, or conduct.

13.2 This release does not apply to claims arising from Direct Freight's own gross negligence, willful misconduct, or violation of applicable law in its own operation of the Platform (as distinct from facilitating a transaction between Users).

13.3 If you are a California resident, you waive California Civil Code Section 1542, which states: "A general release does not extend to claims that the creditor or releasing party does not know or suspect to exist in his or her favor at the time of executing the release, which if known by him or her would have materially affected his or her settlement with the debtor or released party." You understand this release includes claims you do not know or suspect to exist at the time this release becomes effective.

---

## 14. Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW, DIRECT FREIGHT AND ITS OFFICERS, EMPLOYEES, AND AFFILIATES SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, OR ANY LOSS OF PROFITS, REVENUE, DATA, OR BUSINESS, ARISING OUT OF OR RELATED TO YOUR USE OF THE PLATFORM OR ANY TRANSACTION BETWEEN USERS, EVEN IF ADVISED OF THE POSSIBILITY OF SUCH DAMAGES. DIRECT FREIGHT'S TOTAL CUMULATIVE LIABILITY FOR ANY CLAIM ARISING FROM THESE TERMS OR THE PLATFORM SHALL NOT EXCEED THE AMOUNT YOU PAID DIRECT FREIGHT IN SUBSCRIPTION FEES DURING THE ONE (1) MONTH IMMEDIATELY PRECEDING THE EVENT GIVING RISE TO THE CLAIM.

---

## 15. Indemnification

You agree to indemnify, defend, and hold harmless Direct Freight and its officers, employees, and affiliates from any claims, damages, liabilities, and expenses (including reasonable attorneys' fees) arising from: (a) your use of the Platform; (b) your violation of these Terms; (c) any transportation, cargo, payment, or safety dispute between you and another User; or (d) your violation of any applicable law or third-party right.

---

## 16. Dispute Resolution; Governing Law

16.1 **Governing Law.** These Terms, and any dispute arising from them or from your use of the Platform, are governed by the laws of the State of Ohio, without regard to its conflict-of-law principles.

16.2 **Agreement to Arbitrate.** Except for disputes that qualify for small claims court (see 16.5) or claims for injunctive relief to protect intellectual property or Platform security (see 16.6), you and Direct Freight agree that any dispute, claim, or controversy arising out of or relating to these Terms or your use of the Platform will be resolved by **binding, individual arbitration** administered by the American Arbitration Association ("AAA") under its Consumer Arbitration Rules then in effect, rather than in court, except as set forth below. The arbitration will be conducted in Montgomery County, Ohio, or another mutually agreed location, or by video/phone conference where permitted by the AAA rules.

16.3 **Class Action Waiver.** **You and Direct Freight each agree that any arbitration or proceeding will be conducted only on an individual basis and not as a class, collective, consolidated, or representative action.** Neither you nor Direct Freight may bring a claim as a plaintiff or class member in any purported class, collective, or representative proceeding. If this class action waiver is found unenforceable as to a particular claim or request for relief, that claim or request must be severed and brought in court, and all other claims remain subject to arbitration.

16.4 **Arbitration Costs.** Direct Freight will pay AAA filing fees and arbitrator costs for claims under $10,000 brought by an individual User, except where the arbitrator determines the claim is frivolous. Each side bears its own attorneys' fees unless the arbitrator awards fees to the prevailing party under applicable law.

16.5 **Small Claims Carve-Out.** Either party may instead bring an individual claim in small claims court in a jurisdiction with proper venue, so long as the matter remains in that court and is not removed or appealed to a court of general jurisdiction.

16.6 **Injunctive Relief Carve-Out.** Notwithstanding the above, Direct Freight may bring an action in any court of competent jurisdiction for injunctive or equitable relief to protect its intellectual property, confidential information, or the security and integrity of the Platform.

16.7 **Opt-Out.** You may opt out of this arbitration agreement within thirty (30) days of first accepting these Terms by sending written notice to the contact address in Section 18, including your name, account email, and a clear statement that you wish to opt out of arbitration. If you opt out, disputes will proceed in the state or federal courts located in Montgomery County, Ohio, and both parties consent to personal jurisdiction there.

---

## 17. Changes to These Terms

We may update these Terms from time to time. Material changes will be communicated through the Platform or by email prior to taking effect. Continued use of the Platform after changes take effect constitutes acceptance of the updated Terms.

---

## 18. Contact

Questions about these Terms can be directed to: ashton@directfreightco.com

---

*This document is a general-purpose draft and does not constitute legal advice. Freight transportation involves significant regulatory (FMCSA), liability, insurance, and payments-related legal exposure. Have these Terms reviewed by an attorney licensed in your jurisdiction — and familiar with transportation/logistics law specifically — before publishing or relying on them.*
`;

const PRIVACY_FULL_TEXT = `# Direct Freight Privacy Policy

**Last updated: July 27, 2026**

This Privacy Policy describes how Direct Freight Co LLC ("Direct Freight," "we," "us," or "our") collects, uses, and discloses information about users ("you") of the Direct Freight platform ("Platform"). References to "Shipper" mean a user posting freight for transport, and references to "Trucker" or "Carrier" mean a user (owner-operator or trucking company) transporting that freight, consistent with the definitions used in the Terms of Service.

---

## 1. Information We Collect

**Account information:** Name, company name, email address, EIN or SSN (Carriers), physical/mailing address, phone number, and subscription payment information.

**Carrier verification data:** DOT/MC numbers, FMCSA safety records, operating authority status, insurance certificate data, and out-of-service history sourced from third-party verification providers.

**Load and transaction data:** Load details, origin/destination, pricing, bid history, accepted rates, billing records, and Rate Confirmations.

**Location and telematics data:** Live GPS location tracking is **optional by default**. A Carrier may choose to enable real-time location sharing voluntarily, and a Shipper may require live tracking as a condition of a specific load (shown clearly before a Carrier bids or accepts that load). Where tracking is active, your current ZIP code, GPS coordinates, and equipment status (Loaded/Empty) are collected and made visible to the Shipper on that load. Tracking is never required simply to hold an account or use the Platform generally.

**Device and usage data:** IP addresses, browser type, device identifiers, pages visited, and feature interactions, collected through cookies, similar tracking technologies, and analytics tools. Cookies are small data files stored on your device; you can configure your browser to refuse cookies, though some Platform features may not function properly without them.

**Uploaded documents:** Bills of Lading (BOL), Proofs of Delivery (POD), accessorial receipts, and any other documents you upload to the Platform.

**Tax information:** EIN or SSN, legal business name, and other information required for 1099-K tax compliance filings.

---

## 2. How We Use Your Information

- To verify Carrier operating authority, safety records, and insurance coverage
- To operate and provide the Platform's load posting, bidding, messaging, and tracking features
- To generate Rate Confirmations, receipts, and compliance documents
- To process subscription billing and, where applicable, QuickPay payout processing
- To fulfill tax reporting obligations (1099-K filings for Carriers receiving qualifying payments)
- To send transactional notifications (load matches, bid updates, insurance status changes)
- To maintain a legally defensible audit trail of platform agreements and transactions
...
**With other users:** Carriers' names, company names, ratings, equipment types, MC/DOT verification status, and (when a load is active) location data are visible to Shippers as described in this Policy. Shippers' company names, ratings, and average days-to-pay are visible to Carriers.

**With verification providers:** We share Carrier MC/DOT numbers with third-party verification services (e.g., SaferWatch, Highway, FMCSA) to verify authority and insurance.

**With payment processors:** Subscription billing, card payments, and Stripe Connect payout data is shared with our payment processor, Stripe, Inc. Carriers receiving payouts and Shippers making payments must separately agree to Stripe's Connected Account Agreement and Services Agreement, which govern Stripe's own handling of your financial data. We do not store raw card numbers on our servers.

**With tax authorities:** We may report Carrier payment data to the IRS and applicable state agencies as required by law, including via 1099-K filings.

**For legal compliance:** We may disclose information in response to lawful subpoenas, court orders, or government investigations, or to protect Direct Freight's legal rights.

**Direct Freight does not sell your personal information to third parties.**

---

## 4. Data Retention

We retain account information for as long as your account is active plus a commercially reasonable period for legal compliance (typically 7 years for financial records). Location tracking data is retained for 90 days after a load is marked delivered. You may request deletion of your account; however, we may retain certain information as required by law or to resolve disputes.

---

## 5. Security

We implement commercially reasonable administrative, technical, and physical safeguards to protect your information. However, no system is fully secure, and we cannot guarantee that information transmitted through the Platform will not be intercepted.

---

## 6. Your Rights

Depending on your jurisdiction, you may have rights regarding your personal information. If you are a California resident, the California Consumer Privacy Act (as amended by the CPRA) gives you the right to: know what personal information we collect and how it is used; delete personal information we hold about you, subject to certain exceptions (such as information needed for tax or legal compliance); correct inaccurate personal information; opt out of the "sale" or "sharing" of personal information (Direct Freight does not sell personal information, and does not share it for cross-context behavioral advertising); and not be discriminated against for exercising these rights. Residents of other states with comprehensive privacy laws (including Virginia, Colorado, Connecticut, and Utah) have similar rights under their respective state laws.

To exercise any of these rights, contact us at ashton@directfreightco.com. We may need to verify your identity before fulfilling a request. We will respond within the timeframe required by applicable law.

---

## 7. Children

The Platform is not directed at individuals under eighteen (18) years of age. We do not knowingly collect information from minors.

---

## 8. Changes

We may update this Privacy Policy from time to time. Material changes will be communicated through the Platform or by email. Continued use of the Platform after changes take effect constitutes acceptance.

---

## 9. Contact

Direct Freight Co LLC  
7749 Carter Drive, Waynesville, OH  
ashton@directfreightco.com

---

*This document is a general-purpose draft and does not constitute legal advice. Have this Policy reviewed by an attorney familiar with applicable privacy laws (including CCPA if you have California users) before publishing.*
`;

const SHIPPER_FULL_TEXT = `# Direct Freight Shipper Agreement

**Last updated: July 27, 2026**

This Shipper Agreement ("Agreement") governs your access to and use of the Direct Freight Platform as a Shipper (individual shipper or shipping company posting freight for transport). By completing shipper registration, you agree to be bound by this Agreement in addition to Direct Freight's Terms of Service and Privacy Policy.

**Direct Freight is a technology platform only.** Direct Freight is not a freight broker, motor carrier, or freight forwarder; does not select, dispatch, or supervise any Carrier; and is not a party to the transportation contract, Bill of Lading, or Rate Confirmation formed between you and a Carrier. Direct Freight bears no responsibility for cargo loss, damage, delay, accident, or injury arising from the transportation of your freight by a Carrier found through the Platform — such claims are between you and the Carrier (and your respective insurers) only.

---

## 1. Load Posting Accuracy

1.1 You represent and warrant that all information you post about a load — including weight, dimensions, commodity description, freight condition, special handling requirements, hazmat classification, and pickup/delivery details — is accurate and complete to the best of your knowledge at the time of posting.

1.2 You are solely responsible for ensuring the pickup address, delivery address, and contact information provided for each load are correct and current. Direct Freight is not liable for delays, missed pickups, or misdelivery arising from inaccurate or outdated address or contact information you provided.

1.3 If freight is materially different from what was posted (weight, dimensions, condition, or commodity) at the time of pickup, you acknowledge the Carrier may refuse the load, request a rate adjustment, or file a dispute, and that Direct Freight is not responsible for resolving disagreements arising from such discrepancies.

---

## 2. Carrier Selection & Verification Responsibility

2.1 **You are solely responsible for verifying** that any Carrier you select meets your own requirements before tendering freight to them. Direct Freight provides FMCSA authority status, safety rating, and insurance verification tools to assist your decision, but does not guarantee the ongoing accuracy of this third-party data or the Carrier's actual performance.

2.2 You may set minimum requirements (safety rating, MC verification status, minimum carrier star rating, cargo insurance) on a per-load basis, which the Platform will use to inform Carriers of eligibility before they bid. Setting these requirements does not shift liability for Carrier performance to Direct Freight.

2.3 You acknowledge that accepting a bid or claim from a Carrier is your own business decision, made independently, and that Direct Freight does not recommend, endorse, or guarantee any specific Carrier.

---

## 3. Rate Confirmation & Digital Signature

3.1 A binding Rate Confirmation is generated automatically by the Platform upon acceptance of a Carrier's bid or claim on your posted load. The Rate Confirmation incorporates the load details, agreed rate, and the timestamp of the booking event.

3.2 By accepting a bid or posting a load at a listed price that a Carrier claims, you agree that this constitutes your digital signature and binding agreement to the Rate Confirmation terms. No separate wet signature is required.

3.3 You agree to honor the rate agreed upon in the Rate Confirmation and acknowledge that unilaterally reducing payment after a Carrier has performed the haul, absent a documented dispute (cargo damage, no-show, or breach of agreed terms), is a violation of this Agreement.

---

## 4. Detention Pay Authorization

4.1 **Automatic charge authorization.** By posting a load and having a bid or claim accepted through the Platform, you explicitly authorize Direct Freight to charge your linked payment method for verified detention charges calculated under Section 5.4 of the Terms of Service, without requiring separate approval at the time each charge is assessed.

4.2 Detention is calculated automatically via GPS geofence data from the Carrier's device. Two (2) hours from confirmed arrival are free; detention beyond that accrues at $60.00/hour in fifteen (15) minute increments, capped at five (5) hours ($300.00) per stop.

4.3 If you believe a detention charge was assessed in error, you may submit a dispute in writing within seven (7) days of the charge. The geofence log serves as the presumptive record of arrival and departure times absent clear evidence of a GPS or device malfunction.

4.4 You agree not to dispute a properly disclosed, authorized detention charge with your card issuer as "unrecognized" — see Section 8.7 of the Terms of Service, which applies fully to detention charges under this Agreement.

---

## 5. Cargo Claims & Payment Pause Process

5.1 Direct Freight does not hold, insure, or guarantee cargo. All cargo claims are handled directly between you and the Carrier under applicable law and pursuant to the Bill of Lading contract.

5.2 **Direct Freight never takes custody, possession, or control of any load payment funds at any point.** Where the Platform facilitates payment, Direct Freight may, in its discretion, instruct Stripe — a licensed third-party payment processor, not Direct Freight — to pause or delay its release of disputed amounts pending resolution of a documented cargo claim. This is solely an instruction to Stripe's own systems; the funds remain with Stripe throughout, never with Direct Freight. You explicitly agree to this policy and waive any immediate chargeback rights with respect to load payments subject to such a pause. Disputes will be resolved by the parties directly; Direct Freight will instruct Stripe to release the paused amount once both parties confirm resolution or a court order is received.

5.3 You are responsible for your own cargo insurance. Direct Freight does not provide, arrange, or guarantee cargo coverage on your behalf.

5.4 Claim filing deadlines. Any cargo loss or damage claim must be submitted in writing to the Carrier (and to Direct Freight, for informational purposes) within nine (9) months of the delivery date, or, in the event of non-delivery, within nine (9) months of the anticipated delivery date. Any civil lawsuit related to such a claim must be filed within two (2) years of the date the claim is disallowed or otherwise resolved. These deadlines follow standard industry practice under the Carmack Amendment and apply regardless of any Stripe payment-release pause under Section 5.2.

5.5 **Return trip fees and additional pay.** If a Carrier is unable to complete delivery due to a cause attributable to you — including an incorrect address, wrong cargo, a receiver's refusal of the shipment, an inaccessible or closed facility at the scheduled time, or cancellation after pickup — the Carrier may request a return-trip fee through the Platform, and, where the issue is discovered at the point of delivery (such as a wrong address or wrong cargo), may alternatively offer to complete delivery for an additional fee instead of returning the shipment. Any such request requires your affirmative approval before a charge is processed; it is never automatic, and you choose which option (if any) to accept. If you dispute a request, it will be treated as a cargo claim subject to Section 5.1 and resolved directly between you and the Carrier. Direct Freight facilitates the request and, upon your approval, the resulting Stripe charge, but is not a party to the underlying dispute and does not determine fault.

---

## 6. Authorized Corporate Agents

6.1 If you operate under a corporate account (a shipping company), authorized team members registered under that account may post loads, accept bids, upload documents, and manage load activity on behalf of the company.

6.2 The corporate account holder assumes responsibility for ensuring that all actions taken by authorized team members comply with this Agreement.

---

## 7. Payment & Stripe Payment Method

7.1 Payments to Carriers for completed loads, and any detention charges, are processed through **Stripe**. To post loads and release payment, you must maintain a valid payment method on file and agree to Stripe's [Services Agreement](https://stripe.com/legal/ssa). Direct Freight is not a party to your agreement with Stripe.

7.2 **Direct Freight does not guarantee successful payment processing.** If your payment method is declined, expires, or is otherwise invalid at the time a charge is due, Direct Freight is not liable for resulting delays, Carrier disputes, or account restrictions that follow.

7.3 You are responsible for keeping your payment method current and promptly resolving any billing failures. Repeated payment failures may result in load posting restrictions or account suspension.

7.4 **Chargebacks on your own subscription and load payments.** The chargeback acknowledgment, waiver, and account consequences described in Section 8.7–8.9 of the Terms of Service apply fully to your subscription charges, load payments, and detention charges under this Agreement — you agree you will not dispute a properly disclosed charge as unauthorized or unrecognized, and repeated improper chargebacks may result in suspension or termination of your account.

---

## 8. Compliance & Conduct

8.1 You agree to comply with all applicable federal, state, and local laws and regulations regarding the freight you post, including accurate hazmat disclosure, weight limits, and any required permits.

8.2 You agree not to post freight that is illegal, stolen, or misrepresented in nature.

8.3 You agree not to circumvent the Platform by contacting a Carrier found through Direct Freight to arrange the same or substantially similar recurring loads off-Platform in order to avoid subscription fees — see Section 10 of the Terms of Service.

---

## 9. Confidentiality

9.1 Each party agrees to keep confidential any non-public business information disclosed by the other party in connection with this Agreement or the Platform — including rate information, contact details, and business terms not otherwise made visible through normal use of the Platform — and to use such information solely for purposes of completing transactions on the Platform.

9.2 This section does not restrict either party's use of information that is independently developed, already public, or rightfully received from a third party without confidentiality restrictions.

---

## 10. Term & Termination

10.1 This Agreement remains in effect for as long as you maintain an active Shipper account on the Platform.

10.2 You may terminate this Agreement at any time by canceling your subscription and closing your account through the Platform. Cancellation does not relieve you of obligations already accrued, including payment for any load already accepted by a Carrier and in transit at the time of cancellation.

10.3 Direct Freight may suspend or terminate your account, with or without notice, for violation of this Agreement, the Terms of Service, non-payment of subscription fees, or conduct Direct Freight reasonably believes is fraudulent, unsafe, or harmful to other users of the Platform.

10.4 Upon termination, any load already accepted by a Carrier and in transit must still be paid according to its existing Rate Confirmation unless both parties agree otherwise. Sections 5 (Cargo Claims), 7 (Payment), 9 (Confidentiality), 11 (Indemnification), and 12 (Limitation of Liability) survive termination of this Agreement.

---

## 11. Indemnification

11.1 You agree to indemnify, defend, and hold harmless Direct Freight and its officers, employees, and agents from and against any claims, damages, losses, liabilities, and expenses (including reasonable attorneys' fees) arising out of or related to: (a) your breach of this Agreement; (b) inaccurate, misrepresented, or illegal freight you post to the Platform; (c) your violation of any applicable law or regulation; or (d) any dispute between you and a Carrier.

11.2 This indemnification obligation survives termination of this Agreement and applies regardless of whether Direct Freight is named as a party to the underlying claim.

---

## 12. Limitation of Liability

12.1 TO THE FULLEST EXTENT PERMITTED BY APPLICABLE LAW, DIRECT FREIGHT AND ITS OFFICERS, EMPLOYEES, AND AGENTS SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, INCLUDING BUT NOT LIMITED TO LOST PROFITS, LOST BUSINESS, LOSS OF DATA, OR COST OF SUBSTITUTE SERVICES, ARISING OUT OF OR IN CONNECTION WITH THIS AGREEMENT, THE PLATFORM, OR ANY TRANSPORTATION ARRANGED THROUGH IT — WHETHER BASED ON WARRANTY, CONTRACT, TORT (INCLUDING NEGLIGENCE), OR ANY OTHER LEGAL THEORY, AND EVEN IF DIRECT FREIGHT HAS BEEN ADVISED OF THE POSSIBILITY OF SUCH DAMAGES.

12.2 Direct Freight's total liability arising from or related to this Agreement, to the extent liability is not otherwise disclaimed above, shall not exceed the total subscription fees you paid to Direct Freight in the three (3) months preceding the event giving rise to the claim.

12.3 Some jurisdictions do not allow the exclusion or limitation of certain damages, so some of the above limitations may not apply to you. In such cases, Direct Freight's liability will be limited to the fullest extent permitted by applicable law.

---

## 13. General Provisions

13.1 **Force Majeure.** Neither party is liable for any failure or delay in performance resulting from causes beyond that party's reasonable control, including severe weather, natural disaster, government action, labor disputes, or infrastructure or internet service failures.

13.2 **Severability.** If any provision of this Agreement is found unenforceable by a court or arbitrator, that provision shall be limited or eliminated to the minimum extent necessary, and the remaining provisions shall remain in full force and effect.

13.3 **Entire Agreement.** This Agreement, together with the Terms of Service and Privacy Policy incorporated by reference, constitutes the entire agreement between you and Direct Freight regarding your use of the Platform as a Shipper, and supersedes any prior agreements or understandings on this subject.

13.4 **Modification.** Direct Freight may update this Agreement from time to time. Material changes will be communicated via email or in-app notice, and continued use of the Platform after such notice constitutes acceptance of the updated Agreement.

13.5 **Notices.** Direct Freight may provide notices under this Agreement via the email address or in-app messaging associated with your account. You are responsible for keeping your contact information current.

13.6 **Assignment.** You may not assign this Agreement without Direct Freight's prior written consent. Direct Freight may assign this Agreement in connection with a merger, acquisition, or sale of substantially all its assets.

---

## 14. Dispute Resolution; Governing Law

14.1 This Agreement is governed by the laws of the State of Ohio, consistent with the Terms of Service.

14.2 Any dispute arising from this Agreement is subject to the binding arbitration agreement, class action waiver, and dispute resolution procedure set forth in Section 16 of the Terms of Service, which is incorporated into this Agreement by reference.

---

*This document is a general-purpose draft and does not constitute legal advice. Have this Agreement reviewed by a transportation attorney before publication.*
`;

const CARRIER_FULL_TEXT = `# Direct Freight Carrier Agreement

**Last updated: July 27, 2026**

This Carrier Agreement ("Agreement") governs your access to and use of the Direct Freight Platform as a Carrier (owner-operator, fleet driver, or trucking company). By completing carrier registration, you agree to be bound by this Agreement in addition to Direct Freight's Terms of Service and Privacy Policy.

**Direct Freight is a technology platform only.** Direct Freight is not a motor carrier, freight broker, or freight forwarder; does not own or operate any truck; does not employ or dispatch any driver; and is not a party to the transportation contract, Bill of Lading, or Rate Confirmation formed between you and a Shipper. Direct Freight bears no responsibility for cargo loss, damage, delay, accident, or injury arising from your transportation of a shipment found through the Platform — such claims are between you and the Shipper (and your respective insurers) only.

---

## 1. Carrier Eligibility & Verification

1.1 You represent and warrant that you hold active, unrestricted operating authority from the FMCSA under your DOT and/or MC number, and that such authority remains valid and in good standing throughout your use of the Platform.

1.2 You authorize Direct Freight to verify your operating authority, safety rating, and insurance coverage through third-party data providers (including FMCSA's public data systems, and services such as SaferWatch or Highway if and when integrated) from time to time during your use of the Platform. Because this data may be outdated, incomplete, or inaccurate, Direct Freight does not represent that your status is continuously or automatically monitored in real time.

1.3 You must maintain at all times a minimum of **$750,000 in Auto Liability insurance**. **Cargo insurance is not federally required for general freight and is optional** on Direct Freight — however, an individual Shipper may require proof of Cargo insurance as a condition of a specific load, which will be indicated in that load's posted requirements. If a periodic review under Section 1.2 or your own submitted documentation indicates your Auto Liability coverage has lapsed, your account may be suspended, at Direct Freight's discretion, until active coverage is confirmed.

1.4 You must complete the W-9/1099 tax compliance onboarding, including providing your EIN or SSN, prior to accepting any load. Failure to complete this onboarding will lock load acceptance features.

---

## 2. Prohibition on Double-Brokering

2.1 **Strict double-brokering prohibition.** You agree that you will not re-broker, subcontract, or assign any load tendered to you through the Platform to another carrier without the express written consent of the Shipper. "Double-brokering" means accepting a load from a Direct Freight Shipper and then tendering that load to a third-party carrier for actual transport.

2.2 **Automatic account freeze.** If Direct Freight determines, in its sole reasonable judgment, that you have violated Section 2.1, your account will be immediately suspended, and Direct Freight may direct Stripe to pause release of any pending payment connected to the relevant load while the matter is investigated. Direct Freight reserves the right to permanently terminate accounts found to have engaged in double-brokering.

2.3 **Payment pause and recovery.** Where Direct Freight facilitates a payment or QuickPay advance on a load subsequently found to have been double-brokered, you acknowledge Direct Freight's right to seek recovery of disbursed funds, and to instruct Stripe — which alone holds any transaction funds, never Direct Freight — to pause release of any pending amounts until the matter is resolved.

---

## 3. Location Tracking & Real-Time Visibility

3.1 **Tracking is optional by default.** Live GPS location tracking is not required simply to use the Platform or hold a carrier account. You may voluntarily enable real-time location sharing on any load. Where you do, it powers the Platform's ETA, milestone visibility, and automated detention pay features described in Section 7.

3.2 **A specific load may require tracking.** A Shipper may designate a specific load as requiring live GPS tracking as a condition of that load, disclosed clearly before you bid or accept it. By bidding on or accepting a load marked as requiring tracking, you agree to enable and maintain live location tracking for the duration of that haul. You remain free to decline any load that requires tracking you're not willing to provide.

3.3 You may also manually update your current ZIP code and equipment status in the Platform at any time, whether or not live tracking is active or required.

3.4 Your location data (current ZIP, GPS coordinates, Loaded/Empty status) will be shared with the Shipper associated with any active load, and your general availability/proximity will be visible to other Shippers using the Platform's Nearby Capacity feature when you are marked Empty/Available.

---

## 4. Authorized Corporate Agents

4.1 If you operate under a corporate account (a trucking company), authorized dispatchers and fleet administrators registered under that account may execute transactions, upload documents (BOL, POD, accessorials), and manage load assignments on behalf of drivers.

4.2 Drivers operating under a corporate account acknowledge that their parent account administrator holds primary control over profile configuration, load assignments, and account settings. Individual drivers maintain a read-only view of their historical hauls and ratings.

4.3 The corporate account holder (fleet owner/operator) assumes responsibility for ensuring that all actions taken by authorized agents comply with this Agreement.

---

## 5. Cargo Claims & Payment Pause Process

5.1 Direct Freight does not hold, insure, or guarantee cargo. All cargo claims are handled directly between Shipper and Carrier under applicable law and pursuant to the Bill of Lading contract.

5.2 **Direct Freight never takes custody, possession, or control of any load payment or QuickPay funds at any point.** Where the Platform facilitates payment release or QuickPay advances, Direct Freight may, in its discretion, instruct Stripe — a licensed third-party payment processor, not Direct Freight — to pause or delay its release of disputed amounts pending resolution of a documented cargo claim. This is solely an instruction to Stripe's own systems; the funds remain with Stripe throughout, never with Direct Freight. Users explicitly agree to this policy and waive any immediate chargeback rights with respect to load payments subject to such a pause. Disputes will be resolved by the parties directly; Direct Freight will instruct Stripe to release the paused amount once both parties confirm resolution or a court order is received.

5.3 A carrier's maximum liability for cargo claims shall not exceed the Cargo insurance coverage limits actually maintained by the Carrier, if any. Carriers who do not carry Cargo insurance (which is optional under Section 1.3 unless required by a specific Shipper) acknowledge that they remain personally and fully liable for cargo claims under applicable common carrier law, independent of any Stripe payment-release pause described in Section 5.2.

5.4 Claim filing deadlines. Any cargo loss or damage claim must be submitted in writing to the Carrier (and to Direct Freight, for informational purposes) within nine (9) months of the delivery date, or, in the event of non-delivery, within nine (9) months of the anticipated delivery date. Any civil lawsuit related to such a claim must be filed within two (2) years of the date the claim is disallowed or otherwise resolved. These deadlines follow standard industry practice under the Carmack Amendment and apply regardless of any Stripe payment-release pause under Section 5.2.

5.5 **Return trip fees and additional pay.** If you are unable to complete delivery due to a cause attributable to the Shipper — including an incorrect address, wrong cargo, a receiver's refusal of the shipment, an inaccessible or closed facility at the scheduled time, or cancellation after pickup — you may request a return-trip fee through the Platform, reasonably reflecting the cost of the return haul, and, where the issue is discovered at the point of delivery (such as a wrong address or wrong cargo), may alternatively offer to complete delivery for an additional fee instead of returning the shipment. Any such request requires the Shipper's affirmative approval before a charge is processed; it is never automatic, and you are not guaranteed payment or the Shipper's acceptance of either option merely by submitting a request. If the Shipper disputes your request, it will be treated as a cargo claim subject to Section 5.1 and resolved directly between you and the Shipper. Direct Freight facilitates the request and, upon the Shipper's approval, the resulting Stripe charge, but is not a party to the underlying dispute and does not determine fault.

---

## 6. Rate Confirmation & Digital Signature

6.1 A binding Rate Confirmation is generated automatically by the Platform upon load acceptance (whether via "Accept instantly" or an accepted bid). The Rate Confirmation incorporates the load details, agreed rate, carrier MC/DOT number, and the timestamp of the booking event.

6.2 By accepting a load, you agree that this constitutes your digital signature and binding agreement to the Rate Confirmation terms. No separate wet signature is required.

---

## 7. Detention Pay

7.1 **Automated GPS-based tracking.** Detention time is calculated automatically using the Platform's geofence system, triggered when your device's GPS enters or exits a one (1) mile radius of the pickup or delivery facility. You must keep GPS/location tracking active in the Platform while on an active load to be eligible for automated detention pay — Direct Freight cannot verify or pay detention for dwell time that was not captured by the geofence system.

7.2 **Free time and rate.** Each stop includes two (2) free hours from geofence arrival. Detention beyond the free window accrues at **$60.00 per hour**, billed in fifteen (15) minute increments, up to a maximum of five (5) hours ($300.00) per stop. Detention beyond the cap is not automatically billed and must be negotiated directly with the Shipper.

7.3 **Payment.** Verified detention amounts are charged to the Shipper's payment method on file per Section 5.4 of the Terms of Service and paid out to you via Stripe Connect, subject to Section 8 of this Agreement.

7.4 **Disputes.** If you believe detention time was inaccurately recorded, submit a dispute in writing within seven (7) days of the relevant load's delivery date. Direct Freight will review the geofence log, which serves as the presumptive record of arrival and departure times absent clear evidence of a GPS or device malfunction.

---

## 8. Payment & Stripe Connect Payouts

8.1 Payouts to you for completed loads and verified detention are issued through **Stripe Connect**. To receive payment, you must create a Stripe Express account and agree to Stripe's [Connected Account Agreement](https://stripe.com/legal/connect-account). Direct Freight is not a party to your agreement with Stripe.

8.2 **Direct Freight does not guarantee payment.** If a Shipper's payment method is declined, reversed, or otherwise fails, your recourse is against the Shipper directly. Direct Freight is not liable for payment delays or failures caused by Stripe, your bank, or the Shipper's financial institution.

8.3 You are responsible for completing Stripe's identity verification and bank account linking accurately and promptly. Direct Freight is not responsible for payout delays caused by incomplete or inaccurate Stripe onboarding on your part.

8.4 You are solely responsible for your own tax obligations arising from payments received through the Platform, including timely completion of W-9 documentation under Section 1.4 and any applicable 1099-K reporting issued by Stripe.

8.5 **Chargebacks on Your Own Subscription.** Your Carrier subscription fee is charged to your payment method as disclosed at signup and in Section 3 of the Terms of Service. The chargeback acknowledgment, waiver, and account consequences described in Section 8.7–8.9 of the Terms of Service apply fully to your subscription charges — you agree you will not dispute a properly disclosed subscription charge as unauthorized or unrecognized, and repeated improper chargebacks may result in suspension or termination of your account.

---

## 9. Compliance & Conduct

9.1 You agree to comply with all applicable federal, state, and local laws and regulations, including FMCSA hours-of-service rules, weight limits, hazardous materials regulations, and permitting requirements.

9.2 You agree to carry and deliver only cargo described in the load posting unless expressly agreed otherwise with the Shipper in writing.

9.3 You agree not to circumvent the Platform by contacting a Shipper found through Direct Freight to arrange the same or substantially similar recurring loads off-Platform in order to avoid subscription fees — see Section 10 of the Terms of Service.

---

## 10. Confidentiality

10.1 Each party agrees to keep confidential any non-public business information disclosed by the other party in connection with this Agreement or the Platform — including rate information, contact details, and business terms not otherwise made visible through normal use of the Platform — and to use such information solely for purposes of completing transactions on the Platform.

10.2 This section does not restrict either party's use of information that is independently developed, already public, or rightfully received from a third party without confidentiality restrictions.

---

## 11. Independent Contractor Status

11.1 You acknowledge and agree that you are an independent business, and that nothing in this Agreement creates an employment, agency, joint venture, or partnership relationship between you and Direct Freight. Direct Freight does not control the manner, means, route, schedule, or method by which you perform any haul.

11.2 You are solely responsible for your own employees, subcontractors, equipment, fuel, maintenance, insurance, taxes, and all costs of operating your business. Direct Freight does not withhold taxes on your behalf, does not provide benefits, and issues no wages — only facilitates payment from Shippers via Stripe Connect for hauls you independently choose to accept.

11.3 Nothing in this Agreement shall be construed to make Direct Freight a joint employer, co-employer, or otherwise responsible for your relationship with your own drivers or subcontractors, if any.

---

## 12. Term & Termination

12.1 This Agreement remains in effect for as long as you maintain an active Carrier account on the Platform.

12.2 You may terminate this Agreement at any time by canceling your subscription and closing your account through the Platform. Cancellation does not relieve you of obligations already accrued, including completion of any load already accepted and in transit at the time of cancellation.

12.3 Direct Freight may suspend or terminate your account, with or without notice, for violation of this Agreement, the Terms of Service, non-payment of subscription fees, lapsed insurance, revoked operating authority, or conduct Direct Freight reasonably believes is fraudulent, unsafe, or harmful to other users of the Platform.

12.4 Upon termination, any load already accepted and in transit must still be completed and paid according to its existing Rate Confirmation unless both parties agree otherwise. Sections 5 (Cargo Claims), 8 (Payment), 10 (Confidentiality), 11 (Independent Contractor Status), 13 (Indemnification), and 14 (Limitation of Liability) survive termination of this Agreement.

---

## 13. Indemnification

13.1 You agree to indemnify, defend, and hold harmless Direct Freight and its officers, employees, and agents from and against any claims, damages, losses, liabilities, and expenses (including reasonable attorneys' fees) arising out of or related to: (a) your breach of this Agreement; (b) your operation of a commercial motor vehicle, including any accident, injury, or property damage; (c) your violation of any applicable law or regulation; or (d) any dispute between you and a Shipper.

13.2 This indemnification obligation survives termination of this Agreement and applies regardless of whether Direct Freight is named as a party to the underlying claim.

---

## 14. Limitation of Liability

14.1 TO THE FULLEST EXTENT PERMITTED BY APPLICABLE LAW, DIRECT FREIGHT AND ITS OFFICERS, EMPLOYEES, AND AGENTS SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, INCLUDING BUT NOT LIMITED TO LOST PROFITS, LOST BUSINESS, LOSS OF DATA, OR COST OF SUBSTITUTE SERVICES, ARISING OUT OF OR IN CONNECTION WITH THIS AGREEMENT, THE PLATFORM, OR ANY TRANSPORTATION ARRANGED THROUGH IT — WHETHER BASED ON WARRANTY, CONTRACT, TORT (INCLUDING NEGLIGENCE), OR ANY OTHER LEGAL THEORY, AND EVEN IF DIRECT FREIGHT HAS BEEN ADVISED OF THE POSSIBILITY OF SUCH DAMAGES.

14.2 Direct Freight's total liability arising from or related to this Agreement, to the extent liability is not otherwise disclaimed above, shall not exceed the total subscription fees you paid to Direct Freight in the three (3) months preceding the event giving rise to the claim.

14.3 Some jurisdictions do not allow the exclusion or limitation of certain damages, so some of the above limitations may not apply to you. In such cases, Direct Freight's liability will be limited to the fullest extent permitted by applicable law.

---

## 15. General Provisions

15.1 **Force Majeure.** Neither party is liable for any failure or delay in performance resulting from causes beyond that party's reasonable control, including severe weather, natural disaster, government action, labor disputes, or infrastructure or internet service failures.

15.2 **Severability.** If any provision of this Agreement is found unenforceable by a court or arbitrator, that provision shall be limited or eliminated to the minimum extent necessary, and the remaining provisions shall remain in full force and effect.

15.3 **Entire Agreement.** This Agreement, together with the Terms of Service and Privacy Policy incorporated by reference, constitutes the entire agreement between you and Direct Freight regarding your use of the Platform as a Carrier, and supersedes any prior agreements or understandings on this subject.

15.4 **Modification.** Direct Freight may update this Agreement from time to time. Material changes will be communicated via email or in-app notice, and continued use of the Platform after such notice constitutes acceptance of the updated Agreement.

15.5 **Notices.** Direct Freight may provide notices under this Agreement via the email address or in-app messaging associated with your account. You are responsible for keeping your contact information current.

15.6 **Assignment.** You may not assign this Agreement without Direct Freight's prior written consent. Direct Freight may assign this Agreement in connection with a merger, acquisition, or sale of substantially all its assets.

---

## 16. Dispute Resolution; Governing Law

16.1 This Agreement is governed by the laws of the State of Ohio, consistent with the Terms of Service.

16.2 Any dispute arising from this Agreement is subject to the binding arbitration agreement, class action waiver, and dispute resolution procedure set forth in Section 16 of the Terms of Service, which is incorporated into this Agreement by reference.
...
// Renders the real, complete legal document text (Markdown-ish format) into
// styled React elements — used so the actual finalized Terms of Service,
// Privacy Policy, Shipper Agreement, and Carrier Agreement can be shown
// in full within the app, not just a summary.
function renderLegalMarkdown(text) {
  ...
  const flushList = () => {
    if (listBuffer.length) {
      elements.push(<ul key={key++} style={{ margin: "6px 0 12px 0", paddingLeft: 20 }}>{listBuffer}</ul>);
      listBuffer = [];
    }
  };

  const renderInline = (str) => {
    // Bold **text** and italic *text* — simple, non-nested inline parsing
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith("*This document")) {
      flushList();
      elements.push(<p key={key++} style={{ marginTop: 18, fontSize: 11, color: "#9A958A", fontStyle: "italic" }}>{line.replace(/^\*|\*$/g, "")}</p>);
      continue;
    }
    if (line.startsWith("# ")) { flushList(); elements.push(<h3 key={key++} style={{ fontSize: 16, fontWeight: 700, color: "#1B1D21", marginTop: 4, marginBottom: 10 }}>{line.slice(2)}</h3>); continue; }
    if (line.startsWith("## ")) { flushList(); elements.push(<h4 key={key++} style={termsH4}>{line.slice(3)}</h4>); continue; }
    if (line.startsWith("### ")) { flushList(); elements.push(<h5 key={key++} style={{ ...termsH4, fontSize: 12, marginTop: 12 }}>{line.slice(4)}</h5>); continue; }
    if (line.startsWith("- ") || line.startsWith("* ")) {
      listBuffer.push(<li key={key++} style={{ marginBottom: 4 }}>{renderInline(line.slice(2))}</li>);
      continue;
    }
    if (line.startsWith("**Last updated")) {
      flushList();
      elements.push(<p key={key++} style={{ fontSize: 11, color: "#9A958A", marginTop: 0 }}>{renderInline(line)}</p>);
      continue;
    }
    flushList();
    elements.push(<p key={key++} style={{ margin: "0 0 10px 0" }}>{renderInline(line.replace(/&nbsp;/g, ""))}</p>);
  }
  flushList();
  return elements;
}
...
function TermsModal({ onClose }) {
  return (
    <LegalModal title="Terms of Service" icon={<FileText size={16} color="#FFB400" />} onClose={onClose}>
      {renderLegalMarkdown(TOS_FULL_TEXT)}
    </LegalModal>
  );
}
...
function PrivacyModal({ onClose }) {
  return (
    <LegalModal title="Privacy Policy" icon={<FileText size={16} color="#FFB400" />} onClose={onClose}>
      {renderLegalMarkdown(PRIVACY_FULL_TEXT)}
    </LegalModal>
  );
}
...
function CarrierAgreementModal({ onClose }) {
  return (
    <LegalModal title="Carrier Agreement" icon={<Truck size={16} color="#FF5A1F" />} onClose={onClose}>
      {renderLegalMarkdown(CARRIER_FULL_TEXT)}
    </LegalModal>
  );
}
...
function ShipperAgreementModal({ onClose }) {
  return (
    <LegalModal title="Shipper Agreement" icon={<Package size={16} color="#FFB400" />} onClose={onClose}>
      {renderLegalMarkdown(SHIPPER_FULL_TEXT)}
    </LegalModal>
  );
}
...
function ShipperApp({ me, corp, loads, messages, independentTruckers, corporations, setLoads, updateTrucker, updateShipperProfile, addCorpMember, sendMessage, setMessages, addNotification, onLogout, onSwitchProfile }) {
  ...
  const counterBid = async (loadId, bidId, amount) => {
    setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, bids: l.bids.map((b) => (b.id === bidId ? { ...b, status: "countered", counterAmount: amount, counterBy: "shipper" } : b)) } : l)));
    try {
      await api.patch(`/api/bids/${bidId}`, { status: "countered", counter_amount: amount, counter_by: "shipper" });
    } catch (err) {
      console.warn("Counter-offer not saved to DB:", err.message);
    }
  };
  const rejectBid = async (loadId, bidId) => {
    setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, bids: l.bids.map((b) => (b.id === bidId ? { ...b, status: "rejected" } : b)) } : l)));
    try { await api.patch(`/api/bids/${bidId}`, { status: "rejected" }); }
    catch (err) { console.warn("Bid rejection not saved to DB:", err.message); }
  };
  const resolveReturnTrip = async (loadId, decision) => {
    if (decision === "disputed") {
      setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, returnTripStatus: "disputed", returnTripResolvedAt: Date.now() } : l)));
      try { await api.patch(`/api/loads/${loadId}`, { return_trip_status: "disputed", return_trip_resolved_at: new Date().toISOString() }); }
      catch (err) { console.warn("Return trip dispute not saved to DB:", err.message); }
      return;
    }

    const load = myLoads.find((l) => l.id === loadId);
    const trucker = load ? resolveTrucker(load.truckerId) : null;
    const shipperCustomerId = me.billing?.stripeCustomerId;
    const carrierStripeAccountId = trucker?.payout?.stripeAccountId;
    const resolvedStatus = decision === "additional_pay" ? "resolved_additional_pay" : "approved";
    const amountToCharge = decision === "additional_pay" ? (load.additionalPayFee || 0) : (load.returnTripFee || 0);

    if (!shipperCustomerId || !carrierStripeAccountId) {
      console.warn(!shipperCustomerId
        ? "No real payment charged: shipper hasn't completed Stripe Checkout yet."
        : "No real payment charged: this carrier hasn't connected a Stripe payout account yet.");
      setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, returnTripStatus: resolvedStatus, returnTripResolvedAt: Date.now() } : l)));
      try { await api.patch(`/api/loads/${loadId}`, { return_trip_status: resolvedStatus, return_trip_resolved_at: new Date().toISOString() }); }
      catch (err) { console.warn("Return trip resolution not saved to DB:", err.message); }
      return;
    }

    try {
      await api.post("/api/stripe/pay-load", {
        shipperCustomerId, carrierStripeAccountId,
        amountCents: Math.round(amountToCharge * 100),
        loadId, returnTrip: decision !== "additional_pay",
      });
      setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, returnTripStatus: resolvedStatus, returnTripResolvedAt: Date.now() } : l)));
      await api.patch(`/api/loads/${loadId}`, { return_trip_status: resolvedStatus, return_trip_resolved_at: new Date().toISOString() });
    } catch (err) {
      console.error("Return trip payment failed:", err.message);
      alert("Payment failed — no charge went through: " + err.message + "\n\nThe return trip fee has NOT been marked as paid.");
    }
  };
  const releasePayment = async (loadId) => {
    ...
    try {
      await api.post("/api/stripe/pay-load", {
        ...
        loadId, quickPay: false,
      });
      setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, paid: true, paidAt: Date.now() } : l)));
      await api.patch(`/api/loads/${loadId}`, { paid: true, paid_at: new Date().toISOString() });
    } catch (err) {
      console.error("Real Stripe payment failed:", err.message);
      alert("Payment failed — no charge went through: " + err.message + "\n\nThe load has NOT been marked as paid. Check Stripe's dashboard for details.");
    }
  };
    ...
    <Shell title={me.name} subtitle={corp ? corp.companyName : "Shipper"} avatar={me.avatar} me={me} onLogout={onLogout} onSwitchProfile={onSwitchProfile}
      ...
      {tab === "mine" && (
        <MyLoads loads={myLoads} resolveTrucker={resolveTrucker} onRate={rate} onAcceptBid={acceptBid} onCounterBid={counterBid}
          ...
          onViewInvoice={(load) => setInvoiceLoad(load)} onCancelLoad={cancelLoad} onResolveReturnTrip={resolveReturnTrip} me={me} />
      )}
      {tab === "bidcenter" && (
...
function MyLoads({ loads, resolveTrucker, onRate, onAcceptBid, onCounterBid, onRejectBid, onReleasePayment, onOpenChat, onViewInvoice, onCancelLoad, onResolveReturnTrip, me }) {
              ...
              <div style={{ marginTop: 12, borderTop: "1px solid #EEE8DA", paddingTop: 12 }}>
                <div style={{ fontSize: 13, marginBottom: 6, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    ...
                    <BOLButton load={l} trucker={trucker} shipper={me} />
                ...
                {l.returnTripStatus === "requested" && (
                  <div style={{ marginBottom: 10, background: "#FFF6E5", border: "1px solid #FFD98C", borderRadius: 8, padding: "12px 14px", fontSize: 12 }}>
                    <div style={{ fontWeight: 700, color: "#B5790A", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}><Bell size={14} /> Delivery issue reported</div>
                    <div style={{ color: "#44484D", marginBottom: 4 }}><b>Reason:</b> {l.returnTripReason}</div>
                    {l.returnTripNote && <div style={{ color: "#44484D", marginBottom: 4 }}><b>Details:</b> {l.returnTripNote}</div>}
                    {l.additionalPayFee > 0 ? (
                      <>
                        <div style={{ color: "#44484D", marginBottom: 10 }}>The carrier can still complete this delivery, or bring it back instead — your choice:</div>
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                          <button onClick={() => { if (window.confirm(`Pay ${fmtMoney(l.additionalPayFee)} more and have the carrier complete this delivery as originally planned?`)) onResolveReturnTrip(l.id, "additional_pay"); }} style={{ ...primaryBtn("#3A6EA5"), color: "#fff", padding: "6px 14px", fontSize: 12 }}>Pay {fmtMoney(l.additionalPayFee)} More — Complete Delivery</button>
                          <button onClick={() => { if (window.confirm(`Pay ${fmtMoney(l.returnTripFee)} and have the carrier bring this load back instead?`)) onResolveReturnTrip(l.id, "approved"); }} style={{ ...primaryBtn("#3E7A4B"), color: "#fff", padding: "6px 14px", fontSize: 12 }}>Return It — Pay {fmtMoney(l.returnTripFee)}</button>
                          <button onClick={() => { if (window.confirm("Dispute this request? This will be handled as a standard Cargo Claim between you and the carrier.")) onResolveReturnTrip(l.id, "disputed"); }} style={{ ...ghostBtn, color: "#C0432B", borderColor: "#F0C4BA", padding: "6px 14px", fontSize: 12 }}>Dispute</button>
                        </div>
                      </>
                    ) : (
                      <>
                        <div style={{ color: "#44484D", marginBottom: 10 }}><b>Return trip fee requested:</b> {fmtMoney(l.returnTripFee)}</div>
                        <div style={{ display: "flex", gap: 8 }}>
                          <button onClick={() => { if (window.confirm(`Approve and pay ${fmtMoney(l.returnTripFee)} for this return trip?`)) onResolveReturnTrip(l.id, "approved"); }} style={{ ...primaryBtn("#3E7A4B"), color: "#fff", padding: "6px 14px", fontSize: 12 }}>Approve & Pay</button>
                          <button onClick={() => { if (window.confirm("Dispute this return trip fee? This will be handled as a standard Cargo Claim between you and the carrier.")) onResolveReturnTrip(l.id, "disputed"); }} style={{ ...ghostBtn, color: "#C0432B", borderColor: "#F0C4BA", padding: "6px 14px", fontSize: 12 }}>Dispute</button>
                        </div>
                      </>
                    )}
                  </div>
                )}
                {l.returnTripStatus === "approved" && (
                  <div style={{ marginBottom: 10, fontSize: 12, color: "#3E7A4B", display: "flex", alignItems: "center", gap: 6 }}><CheckCircle2 size={14} /> Return trip fee of {fmtMoney(l.returnTripFee)} approved and paid</div>
                )}
                {l.returnTripStatus === "resolved_additional_pay" && (
                  <div style={{ marginBottom: 10, fontSize: 12, color: "#3A6EA5", display: "flex", alignItems: "center", gap: 6 }}><CheckCircle2 size={14} /> Additional {fmtMoney(l.additionalPayFee)} paid — carrier completing delivery as originally planned</div>
                )}
                {l.returnTripStatus === "disputed" && (
                  <div style={{ marginBottom: 10, fontSize: 12, color: "#C0432B" }}>Return trip fee disputed — handle directly with the carrier as a Cargo Claim.</div>
                )}
                {(trucker.equipmentType || trucker.truckDesc || trucker.maxWeight || (trucker.dims && (trucker.dims.l || trucker.dims.w || trucker.dims.h))) && (
                  <div className="mono" style={{ fontSize: 11, color: "#6B6557", marginBottom: 8, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    {trucker.equipmentType && <span style={{ background: "#EEE8DA", color: "#44484D", padding: "2px 7px", borderRadius: 4 }}>{trucker.equipmentType}</span>}
                    {trucker.truckDesc && <span>{trucker.truckDesc}</span>}
                    {trucker.maxWeight ? <span>up to {trucker.maxWeight.toLocaleString()} lbs</span> : ""}
                    {trucker.dims && (trucker.dims.l || trucker.dims.w || trucker.dims.h) ? <span>{trucker.dims.l || "?"}×{trucker.dims.w || "?"}×{trucker.dims.h || "?"} ft (L×W×H)</span> : ""}
                  </div>
                )}
                <TrackerBar load={l} />
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ fontSize: 13 }}>
            ...
            {trucker && (trucker.truckDesc || trucker.maxWeight || (trucker.dims && (trucker.dims.l || trucker.dims.w || trucker.dims.h))) && (
              <div className="mono" style={{ marginTop: 3, fontSize: 11, color: "#6B6557" }}>
                {trucker.truckDesc && <span>{trucker.truckDesc}</span>}
                {trucker.maxWeight ? <span>{trucker.truckDesc ? " · " : ""}up to {trucker.maxWeight.toLocaleString()} lbs</span> : ""}
                {trucker.dims && (trucker.dims.l || trucker.dims.w || trucker.dims.h) ? <span> · {trucker.dims.l || "?"}×{trucker.dims.w || "?"}×{trucker.dims.h || "?"} ft (L×W×H)</span> : ""}
              </div>
            )}
          </div>
...
function meetsLoadRequirements(load, me) {
  ...
  // Equipment, weight, and dimension checks apply to every load, regardless
  // of whether the shipper set any specific extra requirements — a carrier
  // genuinely can't haul freight their truck can't physically carry.
  if (load.equipmentType && me.equipmentType && load.equipmentType !== me.equipmentType) {
    failed.push(`Requires ${load.equipmentType} — you're set up as ${me.equipmentType}`);
  }
  if (me.maxWeight && load.weight && load.weight > me.maxWeight) {
    failed.push(`Load weighs ${load.weight.toLocaleString()} lbs — exceeds your ${me.maxWeight.toLocaleString()} lb capacity`);
  }
  if (me.dims && load.dims) {
    if (me.dims.l && load.dims.l && load.dims.l > me.dims.l) failed.push(`Freight length (${load.dims.l} ft) exceeds your trailer length (${me.dims.l} ft)`);
    if (me.dims.w && load.dims.w && load.dims.w > me.dims.w) failed.push(`Freight width (${load.dims.w} ft) exceeds your trailer width (${me.dims.w} ft)`);
    if (me.dims.h && load.dims.h && load.dims.h > me.dims.h) failed.push(`Freight height (${load.dims.h} ft) exceeds your trailer height (${me.dims.h} ft)`);
  }
...
function StartTrackingButton({ load, me }) {
  const [status, setStatus] = useState("idle"); // idle | requesting | active | denied | error
  const pingRef = useRef(null);
  const facilityCoordRef = useRef(null);

  useEffect(() => () => clearInterval(pingRef.current), []);

  const sendPing = async (lat, lng) => {
    if (!facilityCoordRef.current) return;
    try {
      await fetch(`${API_BASE}/api/tracking/ping`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          loadId: load.id, carrierId: me?.id, lat, lng,
          facilityLat: facilityCoordRef.current.lat, facilityLng: facilityCoordRef.current.lng,
        }),
      });
    } catch (e) {
  ...
  const start = async () => {
    ...
    // Real geocoding for the facility, same accurate source DetentionTracker
    // uses — needed since the backend ping endpoint calculates geofence
    // distance as part of its normal response, even though this button's
    // own purpose is just live map tracking, not detention.
    const result = load.originZip ? await geocodeZip(load.originZip) : null;
    facilityCoordRef.current = result ? { lat: result.lat, lng: result.lng } : { lat: 39.7589, lng: -84.1916 };
  ...
  return (
    <div style={{ marginTop: 10 }}>
      {status === "idle" && (
        <button onClick={start} style={{ ...primaryBtn("#3A6EA5"), color: "#fff", padding: "10px 18px", display: "flex", alignItems: "center", gap: 8 }}>
          <Navigation size={15} /> Start Tracking
        </button>
      )}
      {status === "requesting" && (
        <div style={{ fontSize: 12, color: "#6B6557", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {status === "denied" && (
        <div style={{ fontSize: 12, color: "#C0432B", background: "#FDF1EE", border: "1px solid #F3B7A6", borderRadius: 6, padding: "8px 10px", display: "flex", alignItems: "center", gap: 6 }}>
          <Lock size={13} /> Location permission denied. Enable it in your browser settings to share live tracking.
        </div>
      )}
      {status === "active" && (
        <button onClick={stop} style={{ background: "#F1F8F2", border: "1px solid #BFE0C6", color: "#3E7A4B", borderRadius: 8, padding: "10px 18px", fontWeight: 700, fontSize: 13, display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
          <Navigation size={15} /> Live tracking active — Stop
        </button>
      )}
    </div>
  );
...
function DetentionTracker({ load, me }) {
  // Manual coordinate entry was removed — a carrier could simply look up a
  // facility's real address on Google Maps and type those exact coordinates
  // in without ever actually being there, fraudulently triggering detention
  // pay they weren't entitled to. Real browser GPS is the fix: it detects
  // where the phone actually is, rather than trusting whatever numbers
  // someone chooses to type. This is safe to rely on now that the real bug
  // behind the earlier "621 miles away" readings — the facility's own
  // coordinates coming from a fake geocoder, not GPS inaccuracy — is fixed.
  const [status, setStatus] = useState("idle"); // idle | requesting | denied | active
  const [tracking, setTracking] = useState(null); // last check-in/checkout response from server
  const [elapsed, setElapsed] = useState(0); // seconds since arrival for live clock
  const [checkInError, setCheckInError] = useState("");
  const [checkingIn, setCheckingIn] = useState(false);

  // Live elapsed clock — ticks every second while checked in
  useEffect(() => {
  ...
  // "I've Arrived" — uses the phone's real, live GPS position, not
  // something the carrier can type in themselves.
  const checkIn = () => {
    if (!navigator.geolocation) { setCheckInError("Your browser doesn't support location detection."); return; }
    setCheckInError("");
    setCheckingIn(true);
    setStatus("requesting");
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          const geo = load.originZip ? await geocodeZip(load.originZip) : null;
          const facilityCoord = geo ? { lat: geo.lat, lng: geo.lng } : { lat: 39.7589, lng: -84.1916 }; // fallback: Dayton OH
          const resp = await fetch(`${API_BASE}/api/tracking/ping`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              loadId: load.id, carrierId: me?.id, lat: pos.coords.latitude, lng: pos.coords.longitude,
              facilityLat: facilityCoord.lat, facilityLng: facilityCoord.lng,
          ...
          if (data?.arrivalAt) {
            ...
            setStatus("idle");
            setCheckInError(`Your real GPS location is ${data?.distMiles ?? "several"} miles from the facility — check-in requires being within about 5 miles of the site.`);
          }
        } catch (e) {
          setStatus("idle");
          setCheckInError("Could not reach the server. Check your connection and try again.");
        } finally {
          setCheckingIn(false);
        }
      },
      () => {
        setStatus("denied");
        setCheckingIn(false);
        setCheckInError("Location permission denied. Enable location access in your browser/phone settings to check in — this can't be done manually, since it needs to genuinely confirm you're on-site.");
      }
    );
  };

  // "I've Left" — tells the backend right now, immediately ending detention
  // tracking and calculating the final charge.
  const checkOut = async () => {
    try {
      const resp = await fetch(`${API_BASE}/api/tracking/checkout`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (resp.ok) {
        ...
        if (data.tracked) setTracking(data);
      }
    } catch (e) {
      console.warn("Checkout not saved:", e.message);
    }
  };

  // If the carrier marks the load picked up while checked in, automatically
  // end tracking/detention rather than leaving it running indefinitely.
  useEffect(() => {
    if (load.pickedUpAt && status === "active" && !tracking?.departureAt) {
      checkOut();
    }
  }, [load.pickedUpAt]);
  ...
  return (
    <div style={{ marginTop: 10 }}>
      {(status === "idle" || status === "requesting" || status === "denied") && (
        <div style={{ border: load.requirements?.requireGpsTracking ? "1px solid #FFD98C" : "1px solid #E2DCCC", background: load.requirements?.requireGpsTracking ? "#FFF6E5" : "#fff", borderRadius: 8, padding: "10px 14px" }}>
          <div style={{ fontSize: 12, color: "#6B6557", display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
            <Navigation size={13} />
            {load.requirements?.requireGpsTracking
              ? <span><b style={{ color: "#B5790A" }}>This shipper requires location check-in</b> for this load — also enables automatic detention pay.</span>
              : "Check in with your real location once you've arrived on-site to enable automatic detention pay for this load."}
          </div>
          <button onClick={checkIn} disabled={checkingIn} style={{ ...primaryBtn("#FFB400"), padding: "9px 16px", display: "flex", alignItems: "center", gap: 6, opacity: checkingIn ? 0.7 : 1 }}>
            <Navigation size={13} /> {status === "requesting" ? "Requesting location…" : "I've Arrived"}
          </button>
          <div style={{ fontSize: 10, color: "#9A958A", marginTop: 6 }}>
            Uses your phone's real location — this can't be entered manually, since it needs to genuinely confirm you're on-site.
          </div>
          {checkInError && <div style={{ fontSize: 11, color: "#C0432B", marginTop: 8 }}>{checkInError}</div>}
        </div>
      )}

      {status === "active" && (
        <div style={{ border: `2px solid ${detentionActive ? "#C0432B" : "#3E7A4B"}`, borderRadius: 8, overflow: "hidden" }}>
          <div style={{ background: detentionActive ? "#FDF1EE" : "#F1F8F2", padding: "10px 14px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 6 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, fontWeight: 700, fontSize: 13, color: detentionActive ? "#C0432B" : "#3E7A4B" }}>
                <Navigation size={14} /> Checked in on-site
              ...
              {tracking?.departureAt ? (
                <span style={{ fontSize: 11, color: "#3E7A4B", fontWeight: 600 }}>✓ Checked out</span>
              ) : (
                <button onClick={checkOut} style={{ ...ghostBtn, fontSize: 11, padding: "4px 8px" }}>I've Left</button>
              )}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 12 }}>
              ...
              {!detentionActive && !tracking?.departureAt && freeRemaining > 0 && (
              ...
              {tracking?.departureAt && (
                <div style={{ marginTop: 6, background: "#F1F8F2", border: "1px solid #BFE0C6", borderRadius: 6, padding: "8px 10px" }}>
                  <div style={{ fontWeight: 700, color: "#3E7A4B", display: "flex", alignItems: "center", gap: 6 }}>
                    <CheckCircle2 size={14} /> Left {new Date(tracking.departureAt).toLocaleTimeString()}
                    {detentionAmt > 0
...
function TrackingMapWidget({ load }) {
  ...
  // Real embedded map (OpenStreetMap — no API key or billing required,
  // unlike Google Maps) centered on the carrier's actual live coordinates.
  const zoomDegrees = 0.06; // roughly a several-mile view around the truck
  const bbox = [
    position.lng - zoomDegrees,
    position.lat - zoomDegrees,
    position.lng + zoomDegrees,
    position.lat + zoomDegrees,
  ].join(",");
  const mapEmbedUrl = `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&marker=${position.lat},${position.lng}&layer=mapnik`;
  const mapLinkUrl = `https://www.openstreetmap.org/?mlat=${position.lat}&mlon=${position.lng}#map=13/${position.lat}/${position.lng}`;

  return (
    <div style={{ marginTop: 10, border: "1px solid #E2DCCC", borderRadius: 8, overflow: "hidden", background: "#EAF1F8" }}>
      <div style={{ position: "relative" }}>
        <iframe
          title="Live carrier location"
          src={mapEmbedUrl}
          style={{ width: "100%", height: 220, border: "none", display: "block", filter: position.stale ? "grayscale(0.6)" : "none" }}
          loading="lazy"
        />
        {position.stale && (
          <div style={{ position: "absolute", top: 8, left: 8, background: "rgba(27,29,33,0.85)", color: "#FFB400", fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 5 }}>
            LAST SEEN {Math.round(position.ageSeconds / 60)} MIN AGO
          </div>
        )}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", background: "#fff", borderTop: "1px solid #E2DCCC", flexWrap: "wrap", gap: 6 }}>
        <span style={{ fontSize: 11, color: position.stale ? "#B5790A" : "#3E7A4B", display: "flex", alignItems: "center", gap: 4 }}>
          ...
          {position.stale ? "GPS signal lost" : "Live GPS position"}
        </span>
        <a href={mapLinkUrl} target="_blank" rel="noopener noreferrer" className="mono" style={{ fontSize: 10, color: "#3A6EA5", textDecoration: "underline" }}>
          {position.lat.toFixed(4)}, {position.lng.toFixed(4)} — open full map ↗
        </a>
      </div>
    </div>
  );
...
function MessagesInbox({ me, onOpenThread }) {
  const [threads, setThreads] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    api.get(`/api/messages/inbox/${me.id}`)
      .then((res) => { if (!cancelled) setThreads(res.threads || []); })
      .catch((err) => console.warn("Could not load message inbox:", err.message))
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [me.id]);

  if (loading) return <div style={{ color: "#9A958A", fontSize: 13 }}>Loading messages…</div>;
  if (!threads.length) return <Empty text="No messages yet. Conversations about a load, or messages from a shipper reaching out through Nearby Capacity, will show up here." />;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 640 }}>
      {threads.map((t) => (
        <button
          key={t.loadId}
          onClick={() => onOpenThread(t.loadId, t.lastSenderName)}
          style={{ textAlign: "left", background: "#fff", border: "1px solid #E2DCCC", borderRadius: 8, padding: "12px 16px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}
        >
          <div style={{ minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 2, display: "flex", alignItems: "center", gap: 8 }}>
              {t.lastSenderName}
              {t.isGeneral && <span style={{ fontSize: 10, fontWeight: 600, color: "#3A6EA5", background: "#EAF1F8", padding: "1px 6px", borderRadius: 4 }}>General inquiry</span>}
            </div>
            <div style={{ fontSize: 12, color: "#6B6557", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.lastMessage}</div>
          </div>
          <div style={{ fontSize: 11, color: "#9A958A", whiteSpace: "nowrap" }}>{new Date(t.lastSentAt).toLocaleDateString()}</div>
        </button>
      ))}
    </div>
  );
...
function InvoiceModal({ load, shipper, trucker, onClose }) {
  ...
  return (
    <div className="no-print" style={{ position: "fixed", inset: 0, background: "rgba(27,29,33,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 70, padding: 16 }} onClick={onClose}>
      {/* Printing a modal directly usually comes out as a blank/white page —
          the browser tries to print the whole app underneath it, and the
          modal's fixed positioning + clipping don't translate to print media
          at all. This scoped print stylesheet fixes that the standard way:
          hide everything on the page except the invoice content itself, and
          let just that content flow naturally for a real printed page. */}
      <style>{`
        @media print {
      ...
      `}</style>
      <div onClick={(e) => e.stopPropagation()} className="invoice-print-area" style={{ background: "#fff", width: "100%", maxWidth: 640, maxHeight: "90vh", borderRadius: 12, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 12px 40px rgba(0,0,0,0.3)" }}>
        <div className="no-print" style={{ background: "#1B1D21", color: "#F2EDE4", padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
  ...
  return (
    <Card style={{ padding: 24, maxWidth: 520 }}>
      ...
      <div style={{ fontSize: 13, color: "#44484D", marginTop: 14, lineHeight: 1.6 }}>
        {isCorp ? (
          tier.id === "enterprise" ? (
            <>Flat Enterprise rate for fleets/teams of 151–500 profiles — no per-driver or per-seat charges, no custom quote or sales call required. More than 500? <a href="mailto:ashton@directfreightco.com?subject=Enterprise pricing — 500+ profiles" style={{ color: "#B5790A", fontWeight: 700 }}>Contact us</a> for custom pricing.</>
          ) : (
...
function TruckerApp({ me, corp, loads, messages, independentShippers, corporations, setLoads, updateTrucker, updateShipperProfile, addCorpMember, sendMessage, setMessages, addNotification, onLogout, onSwitchProfile }) {
  ...
  const claimAtPrice = async (loadId) => {
    setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, status: "in_transit", truckerId: me.id, progress: 1 } : l)));
    setTab("mine");
    try {
      await api.patch(`/api/loads/${loadId}`, { status: "in_transit", truckerId: me.id, progress: 1 });
    } catch (err) {
      console.warn("Load claim not saved to DB:", err.message);
    }
  };
  ...
  const acceptCounter = async (loadId, bidId) => {
    let acceptedPrice = null;
    setLoads((prev) => prev.map((l) => {
      if (l.id !== loadId) return l;
      const bid = l.bids.find((b) => b.id === bidId);
      acceptedPrice = bid.counterAmount;
    ...
    setTab("mine");
    try {
      await api.patch(`/api/loads/${loadId}`, { status: "in_transit", truckerId: me.id, price: acceptedPrice, progress: 1 });
    } catch (err) {
      console.warn("Counter-offer acceptance not saved to DB:", err.message);
    }
  };
  const reCounter = async (loadId, bidId, amount) => {
    setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, bids: l.bids.map((b) => (b.id === bidId ? { ...b, status: "countered", counterAmount: amount, counterBy: "trucker" } : b)) } : l)));
    try { await api.patch(`/api/bids/${bidId}`, { status: "countered", counter_amount: amount, counter_by: "trucker" }); }
    catch (err) { console.warn("Counter-offer not saved to DB:", err.message); }
  };
  const withdrawBid = async (loadId, bidId) => {
    setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, bids: l.bids.map((b) => (b.id === bidId ? { ...b, status: "rejected" } : b)) } : l)));
    try { await api.patch(`/api/bids/${bidId}`, { status: "rejected" }); }
    catch (err) { console.warn("Bid withdrawal not saved to DB:", err.message); }
  };
  const markPickedUp = async (loadId) => {
    setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, pickedUpAt: Date.now() } : l)));
    try {
      await api.patch(`/api/loads/${loadId}`, { picked_up_at: new Date().toISOString() });
    } catch (err) {
      console.warn("Pickup confirmation not saved to DB:", err.message);
    }
  };
  const markDelivered = async (loadId) => {
    setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, status: "delivered", progress: 100, deliveredAt: Date.now() } : l)));
    try {
      await api.patch(`/api/loads/${loadId}`, { status: "delivered", progress: 100, delivered_at: new Date().toISOString() });
    } catch (err) {
      console.warn("Delivery confirmation not saved to DB:", err.message);
    }
  };
  const reportFailedDelivery = async (loadId, reason, note, fee, additionalPay) => {
    const update = {
      returnTripStatus: "requested", returnTripReason: reason,
      returnTripNote: note, returnTripFee: fee, returnTripRequestedAt: Date.now(),
      additionalPayFee: additionalPay || 0,
    };
    setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, ...update } : l)));
    try {
      await api.patch(`/api/loads/${loadId}`, {
        return_trip_status: "requested", return_trip_reason: reason,
        return_trip_note: note, return_trip_fee: fee,
        return_trip_requested_at: new Date().toISOString(),
        additional_pay_fee: additionalPay || 0,
      });
    } catch (err) {
      console.warn("Return trip request not saved to DB:", err.message);
    }
  };
  ...
  const disconnectPayout = async () => {
    updateTrucker(me.id, (t) => ({ ...t, payout: { connected: false } }));
    await persistTruckerFields({ payout: { connected: false } });
  };
  const saveW9 = async (w9) => {
    updateTrucker(me.id, (t) => ({ ...t, w9 }));
    await persistTruckerFields({ w9 });
  };
  const addDocument = async (loadId, doc) => {
    const existing = loads.find((l) => l.id === loadId);
    const newDocuments = [...(existing?.documents || []), { ...doc, uploadedByName: me.name }];
    setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, documents: newDocuments } : l)));
    try {
      await api.patch(`/api/loads/${loadId}`, { documents: newDocuments });
    } catch (err) {
      console.warn("Document upload not saved to DB:", err.message);
    }
  };
  ...
  const confirmDeliveryStatus = async (loadId, status, zip) => {
    ...
    await persistTruckerFields({ equipmentStatus: status, ...(zip ? { currentZip: zip } : {}) });
    try {
      await api.patch(`/api/loads/${loadId}`, { deliveryStatusConfirmed: true });
    } catch (err) {
      console.warn("Delivery status confirmation not saved to DB:", err.message);
    }
  };
  const requestQuickPay = async (loadId) => {
    const load = myLoads.find((l) => l.id === loadId);
    if (!load) return;
    ...
    if (confirm(`Confirm QuickPay instant payout? You'll receive ${fmtMoney(net)} within minutes (a 1.5% instant payout fee of ${fmtMoney(fee)} applies). Choose Cancel if you'd rather wait for the free standard payout (full ${fmtMoney(load.price)}, 1-2 business days).`)) {
      const carrierStripeAccountId = me.payout?.stripeAccountId;
      if (!carrierStripeAccountId) {
        alert("Connect your Stripe payout account first (from your Profile tab) before requesting QuickPay.");
        return;
      }
      try {
        await api.post("/api/stripe/instant-payout", {
          carrierStripeAccountId,
          amountCents: Math.round(net * 100),
        });
        setLoads((prev) => prev.map((l) => (l.id === loadId ? { ...l, quickPay: true, quickPayFee: fee, quickPayNet: net } : l)));
        try { await api.patch(`/api/loads/${loadId}`, { quick_pay: true }); }
        catch (err) { console.warn("QuickPay request not saved to DB:", err.message); }
      } catch (err) {
        alert("Instant payout couldn't be completed: " + err.message + "\n\nThis can happen if your connected bank/debit card doesn't support instant payouts in test mode, or your Stripe balance isn't available yet. Your standard (free, 1-2 day) payout is unaffected.");
        console.warn("Real instant payout failed:", err.message);
      }
    }
  };
  const [escrowWaiverSeen, setEscrowWaiverSeen] = useState(!!(me.escrowWaiverAccepted));
  const acceptEscrowWaiver = async () => {
    setEscrowWaiverSeen(true);
    updateTrucker(me.id, (t) => ({ ...t, escrowWaiverAccepted: true }));
    try {
      await api.patch(`/api/auth/user/${me.id}`, { escrowWaiverAccepted: true });
    } catch (err) {
      console.warn("Payment terms acknowledgment not saved to DB:", err.message);
    }
  };
  const cancelLoad = async (loadId, reason) => {
  ...
  const tabs = [
    ...
    { id: "messages", label: "Messages",                          shortLabel: "Msgs",     icon: "💬" },
    ...
    <Shell title={me.name} subtitle={corp ? corp.companyName : me.companyLabel} avatar={me.avatar} me={me} onLogout={onLogout} onSwitchProfile={onSwitchProfile}
      ...
      {tab === "mine" && (
        <MyHauls loads={myLoads} me={me} resolveShipper={resolveShipper} onOpenChat={(loadId, shipperId) => { markRead(threadKey(loadId, me.id)); setChatWith({ loadId, shipperId }); }} unreadCounts={unreadCounts}
          onUploadDoc={addDocument} onRateShipper={rateShipper} onConfirmDeliveryStatus={confirmDeliveryStatus} onRequestQuickPay={requestQuickPay} onCancelLoad={cancelLoad} onMarkDelivered={markDelivered} onMarkPickedUp={markPickedUp} onReportFailedDelivery={reportFailedDelivery} />
      )}
      {tab === "messages" && (
        <MessagesInbox
          me={me}
          onOpenThread={(loadId, shipperName) => { markRead(threadKey(loadId, me.id)); setChatWith({ loadId, shipperName }); }}
        />
      )}
          ...
          <Card key={l.id} style={{ padding: 18, borderLeft: overweight || !reqCheck.met ? "4px solid #FF5A1F" : isLaneMatch ? "4px solid #3A6EA5" : "4px solid #FFB400" }}>
            ...
            {!reqCheck.met && (
              <div style={{ marginTop: 8, background: "#FDF1EE", border: "1px solid #F3B7A6", borderRadius: 8, padding: "8px 10px", fontSize: 12, color: "#C0432B" }}>
                <div style={{ fontWeight: 700, marginBottom: 2, display: "flex", alignItems: "center", gap: 5 }}><ShieldCheck size={13} /> You don't meet this load's requirements:</div>
                {reqCheck.failed.map((f, i) => <div key={i}>• {f}</div>)}
                <div style={{ marginTop: 2, color: "#8A3322" }}>Bidding and instant accept are unavailable on this load until these are resolved.</div>
              </div>
            )}
            {myBid ? (
              ...
              <BidForm listedPrice={l.price} miles={l.miles} onClaim={() => onClaim(l.id)} onBid={(amount, note) => onBid(l.id, amount, note)} disabled={!reqCheck.met} />
            )}
          </Card>
        );
      })}
...
function BidForm({ listedPrice, miles, onClaim, onBid, disabled }) {
  ...
  if (disabled) {
    return (
      <div style={{ marginTop: 10, borderTop: "1px solid #EEE8DA", paddingTop: 10 }}>
        <button disabled style={{ ...ghostBtn, opacity: 0.5, cursor: "not-allowed", display: "flex", alignItems: "center", gap: 6 }}>
          <Lock size={13} /> Bidding unavailable — requirements not met
        </button>
      </div>
    );
  }
...
function MyHauls({ loads, me, resolveShipper, onOpenChat, onUploadDoc, onRateShipper, onConfirmDeliveryStatus, onRequestQuickPay, onCancelLoad, onMarkDelivered, onMarkPickedUp, onReportFailedDelivery }) {
      ...
      {loads.map((l) => {
        ...
        <Card key={l.id} style={{ padding: 18 }}>
          ...
          <div style={{ marginTop: 12, borderTop: "1px solid #EEE8DA", paddingTop: 12 }}>
            ...
            <TrackerBar load={l} />
            <StartTrackingButton load={l} me={me} />
            <DetentionTracker load={l} me={me} />
            {l.status === "in_transit" && !l.pickedUpAt && (
              <div style={{ marginTop: 10 }}>
                <button
                  onClick={() => { if (window.confirm("Confirm you've picked up this load from the shipper?")) onMarkPickedUp(l.id); }}
                  style={{ ...primaryBtn("#3A6EA5"), color: "#fff", padding: "10px 18px", display: "flex", alignItems: "center", gap: 8 }}
                >
                  <Package size={15} /> Mark as Picked Up
                </button>
              </div>
            )}
            {l.status === "in_transit" && l.pickedUpAt && (
              <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                <Badge tone="blue">Picked up {new Date(l.pickedUpAt).toLocaleString()}</Badge>
                <button
                  onClick={() => { if (window.confirm("Confirm you've actually delivered this load? This can't be undone.")) onMarkDelivered(l.id); }}
                  style={{ ...primaryBtn("#3E7A4B"), color: "#fff", padding: "10px 18px", display: "flex", alignItems: "center", gap: 8 }}
                >
                  <CheckCircle2 size={15} /> Mark as Delivered
                </button>
                {!l.returnTripStatus && <FailedDeliveryButton load={l} onReport={onReportFailedDelivery} />}
              </div>
            )}
            {l.returnTripStatus && (
              <div style={{ marginTop: 10, background: (l.returnTripStatus === "approved" || l.returnTripStatus === "resolved_additional_pay") ? "#EAF3EC" : l.returnTripStatus === "disputed" ? "#FBEAE7" : "#FFF6E5", border: `1px solid ${(l.returnTripStatus === "approved" || l.returnTripStatus === "resolved_additional_pay") ? "#B8DDC0" : l.returnTripStatus === "disputed" ? "#F0C4BA" : "#FFD98C"}`, borderRadius: 8, padding: "12px 14px", fontSize: 12 }}>
                <div style={{ fontWeight: 700, marginBottom: 4 }}>
                  {l.returnTripStatus === "requested" && "⏳ Delivery issue reported — awaiting shipper response"}
                  {l.returnTripStatus === "approved" && "✓ Return trip fee approved and paid"}
                  {l.returnTripStatus === "resolved_additional_pay" && "✓ Shipper paid extra — complete delivery as originally planned"}
                  {l.returnTripStatus === "disputed" && "Request disputed by shipper"}
                </div>
                <div style={{ color: "#44484D" }}>
                  Reason: {l.returnTripReason}
                  {l.returnTripStatus === "resolved_additional_pay" ? ` · Extra paid: ${fmtMoney(l.additionalPayFee)}` : ` · Return trip fee: ${fmtMoney(l.returnTripFee)}`}
                </div>
                {l.returnTripStatus === "disputed" && <div style={{ color: "#6B6557", marginTop: 4 }}>This falls under the standard Cargo Claims process — resolve directly with the shipper.</div>}
              </div>
            )}
...
function DeadheadCalculator({ openLoads, myLoads }) {
  ...
  const calcManual = async () => {
    ...
    try {
      // Real geocoding here — this is an explicit, on-demand calculation the
      // carrier is directly relying on to decide whether a load is worth
      // taking, so it should use accurate real coordinates, not the fast
      // approximate ones used for passively sorting the list below.
      const result = await estimateMilesReal(currentZip, targetZip);
      if (!result) { alert("Couldn't look up one of those ZIP codes — double-check they're correct."); return; }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 680 }}>
      <Card style={{ padding: 20 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <button onClick={calcManual} disabled={calculating} style={{ ...primaryBtn("#FF5A1F"), color: "#fff", padding: "10px 18px", opacity: calculating ? 0.7 : 1 }}>
        ...
        {manualResult != null && (
          <div style={{ marginTop: 12, background: "#FDF1EE", border: "1px solid #F3B7A6", borderRadius: 8, padding: "10px 14px", fontWeight: 700, color: "#C0432B", display: "flex", alignItems: "center", gap: 8 }}>
            <Route size={16} /> {manualResult.miles} unpaid deadhead miles
            {manualResult.origin && manualResult.destination && (
              <span style={{ fontWeight: 400, fontSize: 12, color: "#8A3322" }}>({manualResult.origin.city}, {manualResult.origin.state} → {manualResult.destination.city}, {manualResult.destination.state})</span>
            )}
          </div>
        )}
      </Card>
...
function ProfileForm({ me, myLoads, onSave, onConnectPayout, onDisconnectPayout }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 560 }}>
      ...
      <PayoutCard payout={me.payout} onConnect={onConnectPayout} onDisconnect={onDisconnectPayout} userId={me.id} email={me.email} />

      <Card style={{ padding: 24, position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: 14, right: 14, background: "#1B1D21", color: "#FFB400", fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 6, display: "flex", alignItems: "center", gap: 5 }}>
          <Lock size={11} /> COMING SOON
        </div>
        <div style={{ fontFamily: "Oswald, sans-serif", fontWeight: 700, fontSize: 18, textTransform: "uppercase", marginBottom: 6, color: "#9A958A" }}>Factoring Company</div>
        <div style={{ fontSize: 12, color: "#9A958A", marginBottom: 16, lineHeight: 1.5 }}>
          Register your factoring company so it automatically appears as the payment remit-to party on your Rate Confirmations and Invoices. This feature is coming soon — check back shortly.
        </div>
        <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "not-allowed", opacity: 0.5 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: "#9A958A" }}>I factor my invoices through a factoring company</span>
        </label>
      </Card>
...
function PayoutCard({ payout, onConnect, onDisconnect, userId, email }) {
  const [connecting, setConnecting] = useState(false);

  // Real Stripe Connect — redirects to the actual backend endpoint, which
  // creates/reuses a real Express account and sends the carrier to Stripe's
  // genuine onboarding page. onConnect is no longer called here directly —
  // the real "connected" status only gets set once Stripe confirms it,
  // after the carrier is redirected back (see the app-level useEffect that
  // handles ?stripe_connect=success/incomplete/error).
  const connect = () => {
    if (!userId) { alert("Could not find your account ID — try refreshing the page."); return; }
    setConnecting(true);
    startStripeConnect(userId, null, email);
  };
    ...
    <Card style={{ padding: 24 }}>
      ...
      {payout?.connected ? (
        ...
        <button onClick={connect} disabled={connecting} style={{ background: "#635BFF", color: "#fff", border: "none", borderRadius: 8, padding: "10px 16px", fontWeight: 700, fontSize: 13, display: "flex", alignItems: "center", gap: 8, cursor: "pointer", opacity: connecting ? 0.7 : 1 }}>
          <CreditCard size={15} /> {connecting ? "Redirecting to Stripe…" : "Connect with Stripe"}
        </button>
      )}
    </Card>
  );
...
function RateConButton({ load, trucker, shipper }) {
  ...
  const rateConFields = {
    ...
    mc_dot: trucker?.corp?.mcNumber || trucker?.mcNumber || "Not on file",
    ...
    rate_per_mile: load.miles ? `$${(load.price / load.miles).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}/mi` : "—",
  ...
  return (
    <>
      ...
      {show && (
        <div className="no-print" style={{ position: "fixed", inset: 0, background: "rgba(27,29,33,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 60, padding: 16 }} onClick={() => setShow(false)}>
          <style>{`
            @media print {
              .no-print, .no-print * { visibility: hidden !important; }
              .ratecon-print-area, .ratecon-print-area * { visibility: visible !important; }
              .ratecon-print-area {
                position: absolute !important; left: 0 !important; top: 0 !important;
                width: 100% !important; max-width: 100% !important; max-height: none !important;
                overflow: visible !important; box-shadow: none !important; border-radius: 0 !important;
          ...
          `}</style>
          <div onClick={(e) => e.stopPropagation()} className="ratecon-print-area" style={{ background: "#fff", width: "100%", maxWidth: 560, borderRadius: 12, overflow: "hidden", boxShadow: "0 12px 40px rgba(0,0,0,0.3)", maxHeight: "90vh", overflowY: "auto" }}>
            <div className="no-print" style={{ padding: "14px 20px", borderBottom: "1px solid #E2DCCC", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#1B1D21", color: "#F2EDE4" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <tbody>
                  {[
                    ...
                    ["MC / DOT", trucker?.corp?.mcNumber || trucker?.mcNumber || "Not on file"],
                    ["Origin", load.origin + (load.originZip ? ` (${load.originZip})` : "")],
                    ["Destination", load.destination + (load.destZip ? ` (${load.destZip})` : "")],
                    ["Miles", load.miles ? `${load.miles} mi` : "—"],
                    ["Commodity", load.description || "—"],
                    ...
                    ["Rate", fmtMoney(load.price)],
                    ["Rate per mile", load.miles ? `$${(load.price / load.miles).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}/mi` : "—"],
...
// ====================================================================
// Lets a shipper generate a real, complete Bill of Lading for a load if they
// don't already have their own — auto-fills everything the load already
// knows (shipper, carrier, origin/destination, commodity, weight), and asks
// for the few things a load doesn't capture (package count, packaging type)
// before producing a printable, professional BOL.
// Lets a carrier report a failed delivery and, where the cause was the
// shipper's own error, request a fair return-trip fee — using the load's
// original rate as the default, since hauling it back is the same trip in
// reverse. This is NOT automatic like detention pay: the shipper must
// actively approve it before any charge processes, since "whose fault" a
// failed delivery was is inherently more judgment-based than a GPS
// timestamp. A disputed request simply falls under the existing Cargo
// Claims process already covered in the Carrier and Shipper Agreements.
//
// For a wrong address or wrong cargo discovered AT delivery specifically,
// the carrier can offer the shipper a real choice on the spot: pay more to
// complete delivery as-is, or have the carrier bring it back instead.
const AT_DELIVERY_CHOICE_REASONS = [
  "Wrong address provided by shipper",
  "Wrong cargo/product loaded",
];
const RETURN_ONLY_REASONS = [
  "Receiver refused shipment",
  "Facility closed or inaccessible at scheduled time",
  "Shipper cancelled after pickup",
];
const SHIPPER_FAULT_REASONS = [...AT_DELIVERY_CHOICE_REASONS, ...RETURN_ONLY_REASONS];
const OTHER_REASONS = ["Other — not shipper's fault"];

function FailedDeliveryButton({ load, onReport }) {
  const [show, setShow] = useState(false);
  const [reason, setReason] = useState(SHIPPER_FAULT_REASONS[0]);
  const [note, setNote] = useState("");
  const [returnFee, setReturnFee] = useState(load.price || 0);
  const [additionalPay, setAdditionalPay] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const isShipperFault = SHIPPER_FAULT_REASONS.includes(reason);
  const isAtDeliveryChoice = AT_DELIVERY_CHOICE_REASONS.includes(reason);

  const submit = async () => {
    setSubmitting(true);
    await onReport(
      load.id, reason, note,
      isShipperFault ? Number(returnFee) || 0 : 0,
      isAtDeliveryChoice ? Number(additionalPay) || 0 : 0
    );
    setSubmitting(false);
    setShow(false);
  };
  ...
  return (
    <>
      <button onClick={() => setShow(true)} style={{ ...ghostBtn, color: "#C0432B", borderColor: "#F0C4BA", display: "flex", alignItems: "center", gap: 6, padding: "10px 14px" }}>
        <Bell size={14} /> Report Delivery Issue
      </button>
      {show && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(27,29,33,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 60, padding: 16 }} onClick={() => setShow(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", width: "100%", maxWidth: 480, borderRadius: 12, overflow: "hidden", boxShadow: "0 12px 40px rgba(0,0,0,0.3)" }}>
            <div style={{ padding: "14px 20px", borderBottom: "1px solid #E2DCCC", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#1B1D21", color: "#F2EDE4" }}>
              <div style={{ fontWeight: 700, fontSize: 15 }}>Report a Failed Delivery</div>
              <button onClick={() => setShow(false)} style={{ background: "none", border: "none", color: "#F2EDE4" }}><X size={18} /></button>
            </div>
            <div style={{ padding: 20, fontSize: 13 }}>
              <Field label="What happened?">
                <select value={reason} onChange={(e) => setReason(e.target.value)} style={{ ...inputStyle, width: "100%" }}>
                  {SHIPPER_FAULT_REASONS.map((r) => <option key={r}>{r}</option>)}
                  {OTHER_REASONS.map((r) => <option key={r}>{r}</option>)}
                </select>
              </Field>
              <Field label="Additional details">
                <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3} placeholder="Briefly describe what happened…" style={{ ...inputStyle, width: "100%", resize: "vertical" }} />
              </Field>
              {isAtDeliveryChoice ? (
                <>
                  <Field label="Additional amount to complete delivery as-is (optional)">
                    <input type="number" min="0" step="0.01" value={additionalPay} onChange={(e) => setAdditionalPay(e.target.value)} placeholder="e.g. 150 for extra miles/handling" style={{ ...inputStyle, width: 220 }} />
                  </Field>
                  <div style={{ fontSize: 11, color: "#6B6557", marginTop: 4, marginBottom: 12 }}>
                    If you can still complete this delivery for more pay (extra miles to the right address, extra handling for the wrong cargo), enter that amount. Leave blank if you can only bring it back.
                  </div>
                  <Field label="Return trip fee (if shipper prefers to have it brought back)">
                    <input type="number" min="0" step="0.01" value={returnFee} onChange={(e) => setReturnFee(Math.max(0, Number(e.target.value) || 0))} style={{ ...inputStyle, width: 160 }} />
                  </Field>
                  <div style={{ fontSize: 11, color: "#6B6557", marginTop: 4 }}>
                    Defaulted to this load's original rate (${load.price?.toLocaleString() || 0}).
                  </div>
                  <div style={{ fontSize: 11, color: "#6B6557", marginTop: 10, background: "#FBF9F4", border: "1px solid #E2DCCC", borderRadius: 6, padding: "8px 10px" }}>
                    {additionalPay ? "The shipper will see both options and choose one — pay more to complete delivery, or have it returned. " : ""}Nothing is charged automatically — the shipper decides.
                  </div>
                </>
              ) : isShipperFault ? (
                <>
                  <Field label="Return trip fee requested">
                    <input type="number" min="0" step="0.01" value={returnFee} onChange={(e) => setReturnFee(Math.max(0, Number(e.target.value) || 0))} style={{ ...inputStyle, width: 160 }} />
                  </Field>
                  <div style={{ fontSize: 11, color: "#6B6557", marginTop: 4 }}>
                    Defaulted to this load's original rate (${load.price?.toLocaleString() || 0}) — hauling it back is essentially the same trip in reverse. You can adjust this before sending.
                  </div>
                  <div style={{ fontSize: 11, color: "#6B6557", marginTop: 10, background: "#FBF9F4", border: "1px solid #E2DCCC", borderRadius: 6, padding: "8px 10px" }}>
                    The shipper must approve this before you're paid — it isn't automatic. If they dispute it, this becomes a standard Cargo Claim to resolve directly with them.
                  </div>
                </>
              ) : (
                <div style={{ fontSize: 11, color: "#6B6557", marginTop: 4 }}>
                  This will be logged on the load's record but won't request a return-trip fee, since the reason selected isn't attributed to the shipper.
                </div>
              )}
              <div style={{ marginTop: 16, display: "flex", gap: 8 }}>
                <button onClick={submit} disabled={submitting} style={{ ...primaryBtn("#C0432B"), color: "#fff", opacity: submitting ? 0.7 : 1 }}>
...
function BOLButton({ load, trucker, shipper }) {
  const [show, setShow] = useState(false);
  const [packageCount, setPackageCount] = useState("");
  const [packageType, setPackageType] = useState("Pallets");
  const bolNum = "BOL-" + load.id.slice(-6).toUpperCase();
  const today = new Date().toLocaleDateString();
      ...
      {show && (
        <div className="no-print" style={{ position: "fixed", inset: 0, background: "rgba(27,29,33,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 60, padding: 16 }} onClick={() => setShow(false)}>
          <style>{`
            @media print {
              .no-print, .no-print * { visibility: hidden !important; }
              .bol-print-area, .bol-print-area * { visibility: visible !important; }
              .bol-print-area {
                position: absolute !important; left: 0 !important; top: 0 !important;
                width: 100% !important; max-width: 100% !important; max-height: none !important;
                overflow: visible !important; box-shadow: none !important; border-radius: 0 !important;
          ...
          `}</style>
          <div onClick={(e) => e.stopPropagation()} className="bol-print-area" style={{ background: "#fff", width: "100%", maxWidth: 560, borderRadius: 12, overflow: "hidden", boxShadow: "0 12px 40px rgba(0,0,0,0.3)", maxHeight: "90vh", overflowY: "auto" }}>
            <div className="no-print" style={{ padding: "14px 20px", borderBottom: "1px solid #E2DCCC", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#1B1D21", color: "#F2EDE4" }}>
              <div style={{ fontWeight: 700, fontSize: 15, display: "flex", alignItems: "center", gap: 8 }}><FileText size={16} color="#FFB400" /> Bill of Lading — {bolNum}</div>
              <button onClick={() => setShow(false)} style={{ background: "none", border: "none", color: "#F2EDE4" }}><X size={18} /></button>
            </div>

            {/* Editable fields the load itself doesn't capture — filled in before printing */}
            <div className="no-print" style={{ padding: "16px 20px", borderBottom: "1px solid #E2DCCC", background: "#FBF9F4", display: "flex", gap: 10, flexWrap: "wrap" }}>
              <Field label="Package count"><input type="number" min="0" value={packageCount} onChange={(e) => setPackageCount(e.target.value === "" ? "" : String(Math.max(0, Number(e.target.value))))} placeholder="e.g. 12" style={{ ...inputStyle, width: 100 }} /></Field>
              <Field label="Packaging type">
                <select value={packageType} onChange={(e) => setPackageType(e.target.value)} style={{ ...inputStyle, width: 140 }}>
                  <option>Pallets</option><option>Boxes</option><option>Crates</option><option>Drums</option><option>Rolls</option><option>Bundles</option><option>Loose/Bulk</option>
                </select>
              </Field>
            </div>

            <div style={{ padding: 24, fontSize: 13, lineHeight: 1.7, color: "#1B1D21" }}>
              <div className="mono" style={{ fontSize: 10, color: "#9A958A", marginBottom: 16 }}>STRAIGHT BILL OF LADING — {today}</div>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <tbody>
                  {[
                    ["BOL Number", bolNum],
                    ...
                    ["Carrier", (trucker?.name || "") + (trucker?.companyLabel ? ` / ${trucker.companyLabel}` : "")],
                    ["MC / DOT", trucker?.corp?.mcNumber || trucker?.mcNumber || "Not on file"],
                    ["Origin (Ship From)", load.origin + (load.originZip ? ` (${load.originZip})` : "")],
                    ["Destination (Ship To)", load.destination + (load.destZip ? ` (${load.destZip})` : "")],
                    ["Commodity", load.description || "—"],
                    ["Weight", load.weight ? `${load.weight.toLocaleString()} lbs` : "—"],
                    ["Packages", packageCount ? `${packageCount} ${packageType}` : "—"],
                    ["Special instructions", load.special || "None"],
                    ["Freight charges", "Prepaid — processed via Direct Freight Platform"],
                  ].map(([k, v]) => (
                    <tr key={k} style={{ borderBottom: "1px solid #EEE8DA" }}>
                      <td style={{ padding: "5px 8px", color: "#6B6557", fontWeight: 600, width: "38%" }}>{k}</td>
                      <td style={{ padding: "5px 8px" }}>{v}</td>
              ...
              <div style={{ marginTop: 18, fontSize: 11, color: "#6B6557", lineHeight: 1.5, borderTop: "1px solid #EEE8DA", paddingTop: 12 }}>
                This is a standard Straight Bill of Lading, generated by Direct Freight as a convenience where the Shipper has not supplied their own. It is not a negotiable instrument. Subject to the terms and conditions of the Uniform Straight Bill of Lading, and to applicable federal law governing carrier liability for loss, damage, or delay (49 U.S.C. § 14706, the "Carmack Amendment"). Direct Freight is a technology platform only, is not a party to this shipping contract, and does not assume carrier or shipper liability under this document.
              </div>

              <div style={{ marginTop: 20, display: "flex", gap: 24, flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 200 }}>
                  <div style={{ borderTop: "1px solid #1B1D21", marginTop: 30, paddingTop: 4, fontSize: 11, color: "#6B6557" }}>Shipper signature / date</div>
                </div>
                <div style={{ flex: 1, minWidth: 200 }}>
                  <div style={{ borderTop: "1px solid #1B1D21", marginTop: 30, paddingTop: 4, fontSize: 11, color: "#6B6557" }}>Carrier / driver signature / date</div>
                </div>
              </div>
...
function IftaLogPanel({ loads }) {
  ...
  return (
    <Card style={{ padding: 20, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ fontFamily: "Oswald, sans-serif", fontWeight: 700, fontSize: 16, textTransform: "uppercase", marginBottom: 6, display: "flex", alignItems: "center", gap: 8, color: "#9A958A" }}>
        <Route size={16} color="#9A958A" /> IFTA Mileage Log
      </div>
      <div style={{ fontSize: 13, color: "#9A958A", marginBottom: 14, lineHeight: 1.5 }}>
        Mileage from your completed hauls, exportable for quarterly IFTA fuel tax filing. This feature is coming soon — check back shortly.
      </div>
      <div style={{ display: "flex", gap: 12, marginBottom: 14, flexWrap: "wrap", opacity: 0.5, pointerEvents: "none" }}>
        <StatCard label="Total miles (platform)" value="—" icon={<Route size={14} />} />
        <StatCard label="Completed hauls" value="—" icon={<Truck size={14} />} />
      </div>
      <button disabled style={{ ...ghostBtn, display: "flex", alignItems: "center", gap: 6, opacity: 0.5, cursor: "not-allowed", color: "#9A958A" }}>
...
function ProfileLimitModal({ currentCount, onClose, onUpgrade }) {
  ...
  const upgradeLimits = { indie: "add more drivers", starter: "up to 50 drivers", growth: "up to 150 drivers", fleet: "up to 500 drivers" };
...
function OperatorApp({ independentShippers, independentTruckers, corporations, loads, messages,
  ...
  const refreshReviews = () => {
    ...
    api.get("/api/feedback")
      ...
      .catch((err) => console.warn("Could not load reviews:", err.message))
  ...
  const tabs = [
    ...
    { id: "reviews",    label: `Reviews (${reviewsLoading ? "…" : reviews.length})` },
      ...
      <div style={{ padding: "20px 24px" }}>
        ...
        {tab === "overview" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: "#1B1D21", border: "1px solid #2A2D32", borderRadius: 8, padding: 16, fontSize: 12, color: "#9A958A", lineHeight: 1.6 }}>
              <b style={{ color: "#FFB400" }}>Access instructions:</b> This dashboard is only accessible to you. To reach it: press <b>Ctrl + Shift + O</b> anywhere on the site, then enter your operator email and PIN. Your real PIN lives only in Railway's <code>OPERATOR_PIN</code> environment variable — change it there anytime, never in this source code.
            </div>
          </div>
        )}
        ...
        {tab === "reviews" && (
          <div>
            ...
            <OperatorFeedbackPanel reviews={reviews} loading={reviewsLoading} onDelete={async (id) => {
              try {
                await api.del(`/api/feedback/${id}`);
                refreshReviews();
              } catch (err) {
                alert("Could not delete this review: " + err.message);
              }
            }} />
          </div>
        )}
...
function RealRevenuePanel() {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get("/api/operator/subscription-revenue")
      .then(setData)
      .catch((err) => console.warn("Could not load real revenue:", err.message))
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <div style={{ fontSize: 13, color: "#9A958A" }}>Loading real revenue…</div>;
  if (!data) return null;

  const planLabels = { solo_shipper: "Shipper", solo_trucker: "Carrier" };

  return (
    <div style={{ background: "#1B1D21", border: "1px solid #2A2D32", borderRadius: 8, padding: 20 }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: "#F2EDE4", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
        <DollarSign size={14} color="#3E7A4B" /> Real Subscription Revenue
      </div>
      <div style={{ fontSize: 11, color: "#6B6557", marginBottom: 16 }}>
        Actual money genuinely collected via Stripe — not an estimate.
      </div>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: data.recentPayments.length ? 20 : 0 }}>
        <div style={{ background: "#0F1013", border: "1px solid #2A2D32", borderRadius: 8, padding: "14px 18px", minWidth: 170 }}>
          <div style={{ fontSize: 11, color: "#6B6557", textTransform: "uppercase", marginBottom: 4 }}>Total collected</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#3E7A4B", fontFamily: "Oswald, sans-serif" }}>{fmtMoney(data.totalRevenueCents / 100)}</div>
        </div>
        <div style={{ background: "#0F1013", border: "1px solid #2A2D32", borderRadius: 8, padding: "14px 18px", minWidth: 170 }}>
          <div style={{ fontSize: 11, color: "#6B6557", textTransform: "uppercase", marginBottom: 4 }}>This month</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#FFB400", fontFamily: "Oswald, sans-serif" }}>{fmtMoney(data.thisMonthCents / 100)}</div>
        </div>
        <div style={{ background: "#0F1013", border: "1px solid #2A2D32", borderRadius: 8, padding: "14px 18px", minWidth: 170 }}>
          <div style={{ fontSize: 11, color: "#6B6557", textTransform: "uppercase", marginBottom: 4 }}>Payments collected</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#3A6EA5", fontFamily: "Oswald, sans-serif" }}>{data.paymentCount}</div>
        </div>
        {Object.entries(data.byPlan).map(([planId, cents]) => (
          <div key={planId} style={{ background: "#0F1013", border: "1px solid #2A2D32", borderRadius: 8, padding: "14px 18px", minWidth: 170 }}>
            <div style={{ fontSize: 11, color: "#6B6557", textTransform: "uppercase", marginBottom: 4 }}>{planLabels[planId] || planId}</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: "#FF5A1F", fontFamily: "Oswald, sans-serif" }}>{fmtMoney(cents / 100)}</div>
          </div>
        ))}
      </div>
      {data.recentPayments.length > 0 && (
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#9A958A", marginBottom: 8 }}>Recent payments</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {data.recentPayments.map((p) => (
              <div key={p.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#C9C2B3", padding: "6px 0", borderBottom: "1px solid #2A2D32" }}>
                <span>{new Date(p.paid_at).toLocaleDateString()} · {planLabels[p.plan_id] || p.plan_id || "Unknown plan"} {p.billing_cycle ? `(${p.billing_cycle})` : ""}</span>
                <span style={{ fontWeight: 700, color: "#3E7A4B" }}>{fmtMoney(p.amount_cents / 100)}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ background: "#1B1D21", border: "1px solid #2A2D32", borderRadius: 8, padding: "14px 16px", fontSize: 12, color: "#9A958A", lineHeight: 1.6 }}>
        <b style={{ color: "#FFB400" }}>About this estimate:</b> These figures are calculated from actual signup dates and current pricing — they are <b>not</b> real billing data. See "Real Subscription Revenue" above for actual collected revenue straight from Stripe's own records.
      </div>
...
// Reviews are now saved to the real database (see /api/feedback endpoints)
// rather than this in-memory array, which only ever lived in the browser
// and was lost on every refresh.
...
function FeedbackModal({ me, onClose }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      console.warn("Review not saved to DB:", err.message);
    }
...
function OperatorFeedbackPanel({ reviews, loading, onDelete }) {
  ...
  if (loading) return <div style={{ fontSize: 13, color: "#9A958A" }}>Loading reviews…</div>;
      ...
      {reviews.map(r => (
        <div key={r.id} style={{ background: "#1B1D21", border: "1px solid #2A2D32", borderRadius: 8, padding: "14px 16px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
            <div>
              <div style={{ fontWeight: 700, fontSize: 14, color: "#F2EDE4" }}>{r.user_name} <span style={{ fontWeight: 400, fontSize: 12, color: "#9A958A" }}>({r.user_role})</span></div>
              <div style={{ fontSize: 11, color: "#6B6557" }}>{r.user_email} · {new Date(r.submitted_at).toLocaleDateString()}</div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <button
                onClick={() => { if (window.confirm("Permanently delete this review?")) onDelete(r.id); }}
                style={{ background: "none", border: "1px solid #44484D", color: "#9A958A", borderRadius: 6, padding: "4px 8px", cursor: "pointer", display: "flex", alignItems: "center" }}
                title="Delete review"
              >
                <X size={13} />
              </button>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {r.what_to_add && <div style={{ fontSize: 12 }}><span style={{ color: "#FFB400", fontWeight: 700 }}>✨ Add: </span><span style={{ color: "#C9C2B3" }}>{r.what_to_add}</span></div>}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button
          onClick={() => {
            const rows = [["Name", "Email", "Position", "Early Bird", "Converted", "Role", "Company", "Phone"]];
            waitlist.forEach(w => rows.push([
              w.name || "", w.email || "", w.position ?? "",
              w.position <= WAITLIST_LIMIT ? "Yes" : "No",
              w.converted ? "Yes" : "No",
              w.role || "", w.company || "", w.phone || "",
            ]));
            const csv = rows.map(r => r.map(cell => `"${String(cell).replace(/"/g, '""')}"`).join(",")).join("\n");
            const blob = new Blob([csv], { type: "text/csv" });
            const url = URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url; a.download = "directfreight-waitlist.csv"; a.click();
            URL.revokeObjectURL(url);
          }}
          style={{ ...ghostBtn, color: "#F2EDE4", borderColor: "#44484D", display: "flex", alignItems: "center", gap: 6, fontSize: 13 }}
        >
          <Download size={14} /> Export as CSV
        </button>
      </div>
      <div style={{ fontSize: 11, color: "#6B6557", marginTop: -6 }}>
        For the launch announcement itself: import this CSV into a real bulk email tool (Mailchimp's free tier covers up to 500 contacts) — EmailJS is built for one-at-a-time transactional emails, not a mass send to your whole list.
      </div>
...
// ====================================================================
function VerificationBypassToggle() {
  ...
  const refreshStatus = async () => {
    try {
      const res = await api.get("/api/operator/verification-bypass-status");
      setActive(res.active);
      setUntil(res.until);
    } catch (err) {
      console.warn("Could not load verification bypass status:", err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggle = async (enabled) => {
    setToggling(true);
    try {
      const res = await api.post("/api/operator/toggle-verification-bypass", { enabled });
      setActive(res.active);
      setUntil(res.until);
    } catch (err) {
  ...
  return (
    <Card style={{ padding: 24, marginBottom: 16, border: active ? "2px solid #C0432B" : "1px solid #2A2D32" }}>
      <div style={{ fontFamily: "Oswald, sans-serif", fontWeight: 700, fontSize: 18, textTransform: "uppercase", marginBottom: 6, color: active ? "#C0432B" : "#F2EDE4", display: "flex", alignItems: "center", gap: 8 }}>
        {active && <Lock size={16} />} Testing: Skip Real Login Codes
      </div>
      <div style={{ fontSize: 13, color: "#9A958A", marginBottom: 16, lineHeight: 1.5 }}>
        While enabled, <b>any</b> code will work to log into <b>any</b> account — no real email required. Genuinely powerful and only meant for active testing. Automatically turns itself back off after 1 hour either way, so it can never be accidentally left running.
      </div>
      {active ? (
        <div style={{ background: "#FDF1EE", border: "1px solid #F3B7A6", borderRadius: 8, padding: "12px 14px" }}>
          <div style={{ fontWeight: 700, color: "#C0432B", fontSize: 13, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
            ⚠️ BYPASS IS CURRENTLY ACTIVE
          </div>
          <div style={{ fontSize: 12, color: "#8A3322", marginBottom: 10 }}>
            Auto-expires at {until ? new Date(until).toLocaleTimeString() : "—"}.
          </div>
          <button onClick={() => toggle(false)} disabled={toggling} style={{ background: "#C0432B", color: "#fff", border: "none", borderRadius: 8, padding: "9px 16px", fontWeight: 700, fontSize: 13, cursor: "pointer", opacity: toggling ? 0.7 : 1 }}>
            {toggling ? "Turning off…" : "Turn Off Now"}
          </button>
        </div>
      ) : (
        <button onClick={() => toggle(true)} disabled={toggling} style={{ ...primaryBtn("#FFB400"), padding: "10px 18px", opacity: toggling ? 0.7 : 1 }}>
          {toggling ? "Enabling…" : "Enable for 1 Hour"}
        </button>
      )}
    </Card>
  );
...
function ClearLoginLimitTool() {
  ...
  const clearLimit = async () => {
    ...
    setErrorMsg("");
    try {
      await api.post("/api/operator/clear-login-limit", { email: email.trim() });
      setStatus("done");
    } catch (err) {
      setStatus("error");
      setErrorMsg(err.message || "Could not clear the limit.");
    }
  };

  return (
    <Card style={{ padding: 24 }}>
      <div style={{ fontFamily: "Oswald, sans-serif", fontWeight: 700, fontSize: 18, textTransform: "uppercase", marginBottom: 6, color: "#F2EDE4" }}>
        Clear Login Rate Limit
      </div>
      <div style={{ fontSize: 13, color: "#9A958A", marginBottom: 16, lineHeight: 1.5 }}>
        If a real shipper or carrier is locked out from too many login attempts and needs back in right away, enter their email below to immediately reset their limit and any pending code — instead of them waiting out the full 15-minute window.
      </div>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <input
          value={email}
          ...
          onKeyDown={(e) => e.key === "Enter" && clearLimit()}
          style={{ ...inputStyle, background: "#2A2D32", color: "#F2EDE4", border: "1px solid #44484D", flex: 1, minWidth: 220 }}
        />
        <button onClick={clearLimit} disabled={status === "loading" || !email.trim()} style={{ ...primaryBtn("#FFB400"), padding: "10px 18px", opacity: status === "loading" ? 0.7 : 1 }}>
          {status === "loading" ? "Clearing…" : "Clear Limit"}
        </button>
      </div>
      {status === "done" && (
        <div style={{ marginTop: 12, fontSize: 13, color: "#3E7A4B", display: "flex", alignItems: "center", gap: 6 }}>
          <CheckCircle2 size={15} /> Cleared — they can request a fresh code right away.
        </div>
      )}
      {status === "error" && (
        <div style={{ marginTop: 12, fontSize: 13, color: "#C0432B" }}>{errorMsg}</div>
      )}
    </Card>
  );
}
